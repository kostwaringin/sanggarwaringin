...
// points at the wp-madx-cache plugin directory because sometimes file paths are weird. Edge cases,
if ( ! defined( 'WPCACHEHOME' ) ) {
	define( 'WPCACHEHOME', __DIR__ . '/' );
}

if ( defined( 'DISABLE_madxCACHE' ) ) {
	wp_cache_debug( 'DISABLE_madxCACHE set, madx_cache disabled.' );
	$madx_cache_enabled = 0;
}
...
// $blog_cache_dir is used all over the code alongside the madxcache directory but at least with multisite installs it appears to do nothing.
// I started putting everything in cache/madxcache/blogname/path/ a long time ago but never got around to removing the code that used the blogs directory.
if ( $blogcacheid != '' ) {
...
// write a plugin to extend wp-madx-cache!
...
// also look for plugins in wp-content/wp-madx-cache-plugins/
if (
	file_exists( WPCACHEHOME . '../wp-madx-cache-plugins/' ) &&
	is_dir( WPCACHEHOME . '../wp-madx-cache-plugins/' )
) {
	$plugins = glob( WPCACHEHOME . '../wp-madx-cache-plugins/*.php' );
	if ( is_array( $plugins ) ) {
		foreach ( $plugins as $plugin ) {
			if ( is_file( $plugin ) ) {
				require_once $plugin;
...
add_cacheaction( 'madxcache_filename_str', 'wp_cache_check_mobile' );
...
if ( ! isset( $wp_madx_cache_late_init ) || ( isset( $wp_madx_cache_late_init ) && false == $wp_madx_cache_late_init ) ) {
