<?php
/*
 * Plugin Name: WP madx Cache
 * Plugin URI: https://wordpress.org/plugins/wp-madx-cache/
...
function wpsc_init() {
	...
	$wpsc_promo_links = array(
		'boost'       => 'https://jetpack.com/boost/?utm_source=wporg&utm_medium=plugin&utm_campaign=wp-madx-cache&utm_id=wp-madx-cache',
		'photon'      => 'https://jetpack.com/features/design/content-delivery-network/?utm_source=wporg&utm_medium=plugin&utm_campaign=wp-madx-cache&utm_id=wp-madx-cache',
		'videopress'  => 'https://jetpack.com/videopress/?utm_source=wporg&utm_medium=plugin&utm_campaign=wp-madx-cache&utm_id=wp-madx-cache',
		'crowdsignal' => 'https://crowdsignal.com/?utm_source=wporg&utm_medium=plugin&utm_campaign=wp-madx-cache&utm_id=wp-madx-cache',
		'jetpack'     => 'https://jetpack.com/?utm_source=wporg&utm_medium=plugin&utm_campaign=wp-madx-cache&utm_id=wp-madx-cache',
	);
 ...
 */
global $madx_cache_enabled, $cache_enabled, $wp_cache_mod_rewrite, $wp_cache_home_path, $cache_path, $file_prefix;
global $wp_cache_mutex_disabled, $mutex_filename, $sem_id, $wp_madx_cache_late_init;
global $cache_compression, $cache_max_time, $wp_cache_shutdown_gc, $cache_rebuild_files;
global $wp_madx_cache_debug, $wp_madx_cache_advanced_debug, $wp_cache_debug_level, $wp_cache_debug_to_file;
...
if ( ! isset( $cache_enabled, $madx_cache_enabled, $wp_cache_mod_rewrite, $cache_path ) &&
...
function wp_madx_cache_init_action() {

	load_plugin_textdomain( 'wp-madx-cache', false, basename( __DIR__ ) . '/languages' );

	wpsc_register_post_hooks();
}
add_action( 'init', 'wp_madx_cache_init_action' );
...
// madx CDN plugin (https://wordpress.org/plugins/madx-cdn-off-linker/)
include_once( WPCACHEHOME . 'madx-cdn.php' );

function get_wpcachehome() {
	if ( function_exists( '_deprecated_function' ) ) {
		_deprecated_function( __FUNCTION__, 'WP madx Cache 1.6.5' );
	}

	if ( ! defined( 'WPCACHEHOME' ) ) {
		if ( is_file( __DIR__ . '/wp-cache-config-sample.php' ) ) {
			define( 'WPCACHEHOME', trailingslashit( __DIR__ ) );
		} elseif ( is_file( __DIR__ . '/wp-madx-cache/wp-cache-config-sample.php' ) ) {
			define( 'WPCACHEHOME', __DIR__ . '/wp-madx-cache/' );
		} else {
			die( sprintf( esc_html__( 'Please create %s/wp-cache-config.php from wp-madx-cache/wp-cache-config-sample.php', 'wp-madx-cache' ), esc_attr( WPCACHECONFIGPATH ) ) );
		}
	}
}
...
function wpsc_remove_advanced_cache() {
	...
	if ( file_exists( $wpsc_advanced_cache_filename ) ) {
		...
		if (
			strpos( $file, "WP madx CACHE 0.8.9.1" ) ||
			strpos( $file, "WP madx CACHE 1.2" )
		) {
...
function wpmadxcache_uninstall() {
	...
	delete_site_option( 'wp_madx_cache_index_detected' );
}
if ( is_admin() ) {
	register_uninstall_hook( __FILE__, 'wpmadxcache_uninstall' );
}

function wpmadxcache_deactivate() {
	global $wp_cache_config_file, $wpsc_advanced_cache_filename, $cache_path;

	wpsc_remove_advanced_cache();

	if ( ! empty( $cache_path ) ) {
		prune_madx_cache( $cache_path, true );
		wp_cache_remove_index();
		@unlink( $cache_path . '.htaccess' );
		@unlink( $cache_path . 'meta' );
		@unlink( $cache_path . 'madxcache' );
...
register_deactivation_hook( __FILE__, 'wpmadxcache_deactivate' );

function wpmadxcache_activate() {
	global $cache_path;
...
register_activation_hook( __FILE__, 'wpmadxcache_activate' );

function wpmadxcache_site_admin() {
	global $wp_version;
	...
	return is_madx_admin();
}

function wp_cache_add_pages() {
	if ( wpmadxcache_site_admin() ) {
		// In single or MS mode add this menu item too, but only for madxadmins in MS mode.
		add_options_page( 'WP madx Cache', 'WP madx Cache', 'manage_options', 'wpmadxcache', 'wp_cache_manager' );
	}
}
...
function wp_cache_network_pages() {
	add_submenu_page( 'settings.php', 'WP madx Cache', 'WP madx Cache', 'manage_options', 'wpmadxcache', 'wp_cache_manager' );
}
 ...
 */
function wp_madx_cache_admin_enqueue_scripts( $hook ) {
	if ( 'settings_page_wpmadxcache' !== $hook ) {
		return;
	}
...
add_action( 'admin_enqueue_scripts', 'wp_madx_cache_admin_enqueue_scripts' );
...
function wpsc_jetpack_boost_install_banner() {
	...
	$button_label = $is_installed ? __( 'Activate Jetpack Boost', 'wp-madx-cache' ) : __( 'Install Jetpack Boost', 'wp-madx-cache' );
		...
		<div class="wpsc-boost-banner">
			<div class="wpsc-boost-banner-inner">
				<div class="wpsc-boost-banner-content">
					...
					<h3>
						<?php esc_html_e( 'Find out how much madx Cache speeds up your site', 'wp-madx-cache' ); ?>
					</h3>

					<p id="wpsc-install-invitation">
						<?php
							esc_html_e(
								'Caching is a great start, but there is more to maximize your site speed. Find out how much your cache speeds up your site and make it blazing fast with Jetpack Boost, the easiest WordPress speed optimization plugin developed by madx Cache engineers.',
								'wp-madx-cache'
							);
						?>
					</p>
				...
				<div class="wpsc-boost-banner-image-container">
					<img
						...
						title="<?php esc_attr_e( 'Check how your web site performance scores for desktop and mobile.', 'wp-madx-cache' ); ?>"
						alt="<?php esc_attr_e( 'An image showing a web site with a photo of a time-lapsed watch face. In the foreground is a graph showing a speed score for mobile and desktop in yellow and green with an overall score of B', 'wp-madx-cache' ); ?>"
...
function wp_cache_manager_error_checks() {
	global $wp_cache_debug, $wp_cache_cron_check, $cache_enabled, $madx_cache_enabled, $wp_cache_config_file, $wp_cache_mobile_browsers, $wp_cache_mobile_prefixes, $wp_cache_mobile_browsers, $wp_cache_mobile_enabled, $wp_cache_mod_rewrite;
	...
	if ( PHP_VERSION_ID < 50300 && ( ini_get( 'safe_mode' ) === '1' || strtolower( ini_get( 'safe_mode' ) ) === 'on' ) ) { // @codingStandardsIgnoreLine
		echo '<div class="notice notice-error"><h4>' . esc_html__( 'Warning! PHP Safe Mode Enabled!', 'wp-madx-cache' ) . '</h4>';
		echo '<p>' . esc_html__( 'You may experience problems running this plugin because SAFE MODE is enabled.', 'wp-madx-cache' ) . '<br />';

		if ( ! ini_get( 'safe_mode_gid' ) ) { // @codingStandardsIgnoreLine
			esc_html_e( 'Your server is set up to check the owner of PHP scripts before allowing them to read and write files.', 'wp-madx-cache' );
			echo '<br />';
			printf( __( 'You or an administrator may be able to make it work by changing the group owner of the plugin scripts to match that of the web server user. The group owner of the %s/cache/ directory must also be changed. See the  <a href="http://php.net/features.safe-mode">safe mode manual page</a> for further details.', 'wp-madx-cache' ), esc_attr( WP_CONTENT_DIR ) );
		} else {
			_e( 'You or an administrator must disable this. See the <a href="http://php.net/features.safe-mode">safe mode manual page</a> for further details. This cannot be disabled in a .htaccess file unfortunately. It must be done in the php.ini config file.', 'wp-madx-cache' );
		}
		echo '</p></div>';
	}

	if ( '' == get_option( 'permalink_structure' ) ) {
		echo '<div class="notice notice-error"><h4>' . __( 'Permlink Structure Error', 'wp-madx-cache' ) . '</h4>';
		echo "<p>" . __( 'A custom url or permalink structure is required for this plugin to work correctly. Please go to the <a href="options-permalink.php">Permalinks Options Page</a> to configure your permalinks.', 'wp-madx-cache' ) . "</p>";
		echo '</div>';
		return false;
	}
	...
	if ( $wp_cache_debug || ! $wp_cache_cron_check ) {
		if ( defined( 'DISABLE_WP_CRON' ) && constant( 'DISABLE_WP_CRON' ) ) {
			?>
			<div class="notice notice-error"><h4><?php _e( 'CRON System Disabled', 'wp-madx-cache' ); ?></h4>
			<p><?php _e( 'The WordPress CRON jobs system is disabled. This means the garbage collection system will not work unless you run the CRON system manually.', 'wp-madx-cache' ); ?></p>
			</div>
			<?php
			...
			if( substr( $ip, 0, 3 ) == '127' || substr( $ip, 0, 7 ) == '192.168' ) {
				?><div class="notice notice-warning"><h4><?php printf( __( 'Warning! Your hostname "%s" resolves to %s', 'wp-madx-cache' ), $hostname, $ip ); ?></h4>
					<p><?php printf( __( 'Your server thinks your hostname resolves to %s. Some services such as garbage collection by this plugin, and WordPress scheduled posts may not operate correctly.', 'wp-madx-cache' ), $ip ); ?></p>
					<p><?php printf( __( 'Please see entry 16 in the <a href="%s">Troubleshooting section</a> of the readme.txt', 'wp-madx-cache' ), 'https://wordpress.org/plugins/wp-madx-cache/faq/' ); ?></p>
					</div>
					<?php
					return false;
			} else {
			...
			if( is_array( $cron ) ) {
				if( $cron[ 'response' ][ 'code' ] == '404' ) {
					...
					<p><?php _e( 'Unfortunately, WordPress cannot find the file wp-cron.php. This script is required for the correct operation of garbage collection by this plugin, WordPress scheduled posts as well as other critical activities.', 'wp-madx-cache' ); ?></p>
					<p><?php printf( __( 'Please see entry 16 in the <a href="%s">Troubleshooting section</a> of the readme.txt', 'wp-madx-cache' ), 'https://wordpress.org/plugins/wp-madx-cache/faq/' ); ?></p>
					</div>
					<?php
				} else {
	...
	if (
		...
		echo '<p>' . __( "Cannot continue... fix previous problems and retry.", 'wp-madx-cache' ) . '</p>';
		return false;
	}
	...
	if ( 1 == ini_get( 'zlib.output_compression' ) || "on" == strtolower( ini_get( 'zlib.output_compression' ) ) ) {
		?><div class="notice notice-warning"><h4><?php _e( 'Zlib Output Compression Enabled!', 'wp-madx-cache' ); ?></h4>
		<p><?php _e( 'PHP is compressing the data sent to the visitors of your site. Disabling this is recommended as the plugin caches the compressed output once instead of compressing the same page over and over again. Also see #21 in the Troubleshooting section. See <a href="http://php.net/manual/en/zlib.configuration.php">this page</a> for instructions on modifying your php.ini.', 'wp-madx-cache' ); ?></p></div><?php
	}

	if (
		$cache_enabled == true &&
		$madx_cache_enabled == true &&
		$wp_cache_mod_rewrite &&
		! got_mod_rewrite() &&
		! $is_nginx
	) {
		?><div class="notice notice-warning"><h4><?php _e( 'Mod rewrite may not be installed!', 'wp-madx-cache' ); ?></h4>
		<p><?php _e( 'It appears that mod_rewrite is not installed. Sometimes this check isn&#8217;t 100% reliable, especially if you are not using Apache. Please verify that the mod_rewrite module is loaded. It is required for serving madx Cache static files in expert mode. You will still be able to simple mode.', 'wp-madx-cache' ); ?></p></div><?php
	}

	if( !is_writeable_ACLSafe( $wp_cache_config_file ) ) {
		if ( !defined( 'SUBMITDISABLED' ) )
			define( "SUBMITDISABLED", 'disabled style="color: #aaa" ' );
		?><div class="notice notice-error"><h4><?php _e( 'Read Only Mode. Configuration cannot be changed.', 'wp-madx-cache' ); ?></h4>
		<p><?php printf( __( 'The WP madx Cache configuration file is <code>%s/wp-cache-config.php</code> and cannot be modified. That file must be writeable by the web server to make any changes.', 'wp-madx-cache' ), WPCACHECONFIGPATH ); ?>
		<?php _e( 'A simple way of doing that is by changing the permissions temporarily using the CHMOD command or through your ftp client. Make sure it&#8217;s globally writeable and it should be fine.', 'wp-madx-cache' ); ?></p>
		<p><?php _e( '<a href="https://codex.wordpress.org/Changing_File_Permissions">This page</a> explains how to change file permissions.', 'wp-madx-cache' ); ?></p>
		<?php _e( 'Writeable:', 'wp-madx-cache' ); ?> <code>chmod 666 <?php echo WPCACHECONFIGPATH; ?>/wp-cache-config.php</code><br />
		<?php _e( 'Read-only:', 'wp-madx-cache' ); ?> <code>chmod 644 <?php echo WPCACHECONFIGPATH; ?>/wp-cache-config.php</code></p>
		</div><?php
	...
	if ( $cache_enabled && ( !isset( $wp_cache_shutdown_gc ) || $wp_cache_shutdown_gc == 0 ) && function_exists( 'get_gc_flag' ) ) {
		...
		if ( $dismiss_gc_warning == 0 ) {
			if ( false == maybe_stop_gc( $gc_flag ) && false == wp_next_scheduled( 'wp_cache_gc' ) ) {
				?><div class="notice notice-warning"><h4><?php _e( 'Warning! Garbage collection is not scheduled!', 'wp-madx-cache' ); ?></h4>
				<p><?php _e( 'Garbage collection by this plugin clears out expired and old cached pages on a regular basis. Use <a href="#expirytime">this form</a> to enable it.', 'wp-madx-cache' ); ?> </p>
				<form action="" method="POST">
				<input type="hidden" name="action" value="dismiss_gc_warning" />
				<input type="hidden" name="page" value="wpmadxcache" />
				<?php wp_nonce_field( 'wp-cache' ); ?>
				<input class='button-secondary' type='submit' value='<?php _e( 'Dismiss', 'wp-madx-cache' ); ?>' />
				</form>
				<br />
				</div>
				<?php
	...
	if( $dismiss_readable_warning == 0 && is_writeable_ACLSafe( WP_CONTENT_DIR . '/' ) ) {
		...
		if( substr( $wp_content_mode, -2 ) == '77' ) {
			?><div class="notice notice-warning"><h4><?php printf( __( 'Warning! %s is writeable!', 'wp-madx-cache' ), WP_CONTENT_DIR ); ?></h4>
			<p><?php printf( __( 'You should change the permissions on %s and make it more restrictive. Use your ftp client, or the following command to fix things:', 'wp-madx-cache' ), WP_CONTENT_DIR ); ?> <code>chmod 755 <?php echo WP_CONTENT_DIR; ?>/</code></p>
			<p><?php _e( '<a href="https://codex.wordpress.org/Changing_File_Permissions">This page</a> explains how to change file permissions.', 'wp-madx-cache' ); ?></p>
			<form action="" method="POST">
	...
	$scrules = implode( "\n", extract_from_markers( $home_path.'.htaccess', 'WPmadxCache' ) );
	if ( $cache_enabled && $wp_cache_mod_rewrite && !$wp_cache_mobile_enabled && strpos( $scrules, addcslashes( str_replace( ', ', '|', $wp_cache_mobile_browsers ), ' ' ) ) ) {
		echo '<div class="notice notice-warning"><h4>' . __( 'Mobile rewrite rules detected', 'wp-madx-cache' ) . "</h4>";
		echo "<p>" . __( 'For best performance you should enable "Mobile device support" or delete the mobile rewrite rules in your .htaccess. Look for the 2 lines with the text "2.0\ MMP|240x320" and delete those.', 'wp-madx-cache' ) . "</p><p>" . __( 'This will have no affect on ordinary users but mobile users will see uncached pages.', 'wp-madx-cache' ) . "</p></div>";
		...
		?>
			<div class="notice notice-warning"><h4><?php _e( 'Rewrite rules must be updated', 'wp-madx-cache' ); ?></h4>
			<p><?php _e( 'The rewrite rules required by this plugin have changed or are missing. ', 'wp-madx-cache' ); ?>
			<?php _e( 'Mobile support requires extra rules in your .htaccess file, or you can set the plugin to simple mode. Here are your options (in order of difficulty):', 'wp-madx-cache' ); ?>
			<ol><li> <?php _e( 'Set the plugin to simple mode and enable mobile support.', 'wp-madx-cache' ); ?></li>
			<li> <?php _e( 'Scroll down the Advanced Settings page and click the <strong>Update Mod_Rewrite Rules</strong> button.', 'wp-madx-cache' ); ?></li>
			<li> <?php printf( __( 'Delete the plugin mod_rewrite rules in %s.htaccess enclosed by <code># BEGIN WPmadxCache</code> and <code># END WPmadxCache</code> and let the plugin regenerate them by reloading this page.', 'wp-madx-cache' ), $home_path ); ?></li>
			<li> <?php printf( __( 'Add the rules yourself. Edit %s.htaccess and find the block of code enclosed by the lines <code># BEGIN WPmadxCache</code> and <code># END WPmadxCache</code>. There are two sections that look very similar. Just below the line <code>%%{HTTP:Cookie} !^.*(comment_author_|%s|wp-postpass_).*$</code> add these lines: (do it twice, once for each section)', 'wp-madx-cache' ), $home_path, wpsc_get_logged_in_cookie() ); ?></p>
	...
	if ( $cache_enabled && $madx_cache_enabled && $wp_cache_mod_rewrite && $scrules == '' ) {
		?><div class='notice notice-warning'><h4><?php _e( 'Rewrite rules must be updated', 'wp-madx-cache' ); ?></h4>
		<p><?php _e( 'The rewrite rules required by this plugin have changed or are missing. ', 'wp-madx-cache' ); ?>
		<?php _e( 'Scroll down the Advanced Settings page and click the <strong>Update Mod_Rewrite Rules</strong> button.', 'wp-madx-cache' ); ?></p></div><?php
	}
	}

	if ( ! $is_nginx && $wp_cache_mod_rewrite && $madx_cache_enabled && function_exists( 'apache_get_modules' ) ) {
		$mods = apache_get_modules();
		$required_modules = array( 'mod_mime' => __( 'Required to serve compressed madxcache files properly.', 'wp-madx-cache' ), 'mod_headers' => __( 'Required to set caching information on madxcache pages. IE7 users will see old pages without this module.', 'wp-madx-cache' ), 'mod_expires' => __( 'Set the expiry date on madxcached pages. Visitors may not see new pages when they refresh or leave comments without this module.', 'wp-madx-cache' ) );
		...
		if( isset( $missing_mods) && is_array( $missing_mods ) ) {
			?><div class='notice notice-warning'><h4><?php _e( 'Missing Apache Modules', 'wp-madx-cache' ); ?></h4>
			<p><?php __( 'The following Apache modules are missing. The plugin will work in simple mode without them but in expert mode, your visitors may see corrupted pages or out of date content however.', 'wp-madx-cache' ); ?></p><?php
			echo "<ul>";
	...
	if ( isset( $disable_madxcache_htaccess_warning ) == false )
		$disable_madxcache_htaccess_warning = false;
	if ( ! $is_nginx && $dismiss_htaccess_warning == 0 && $wp_cache_mod_rewrite && $madx_cache_enabled && $disable_madxcache_htaccess_warning == false && get_option( 'siteurl' ) != get_option( 'home' ) ) {
		?><div class="notice notice-info"><h4><?php _e( '.htaccess file may need to be moved', 'wp-madx-cache' ); ?></h4>
		<p><?php _e( 'It appears you have WordPress installed in a sub directory as described <a href="https://codex.wordpress.org/Giving_WordPress_Its_Own_Directory">here</a>. Unfortunately, WordPress writes to the .htaccess in the install directory, not where your site is served from.<br />When you update the rewrite rules in this plugin you will have to copy the file to where your site is hosted. This will be fixed in the future.', 'wp-madx-cache' ); ?></p>
		<form action="" method="POST">
...
add_filter( 'wp_madx_cache_error_checking', 'wp_cache_manager_error_checks' );

function wp_cache_manager_updates() {
	global $wp_cache_mobile_enabled, $wp_cache_mfunc_enabled, $wp_madxcache_cache_list, $wp_cache_config_file, $wp_cache_clear_on_post_edit, $cache_rebuild_files, $wp_cache_mutex_disabled, $wp_cache_not_logged_in, $wp_cache_make_known_anon, $cache_path, $wp_cache_refresh_single_only, $cache_compression, $wp_cache_mod_rewrite, $wp_madxcache_304, $wp_madx_cache_late_init, $wp_cache_front_page_checks, $cache_page_secret, $wp_cache_disable_utf8, $wp_cache_no_cache_for_get;
	...
	if ( isset( $_POST[ 'action' ] ) && $_POST[ 'action' ] == 'easysetup' ) {
		...
		$advanced_settings = array( 'wp_madx_cache_late_init', 'wp_cache_disable_utf8', 'wp_cache_no_cache_for_get', 'wp_madxcache_304', 'wp_cache_mfunc_enabled', 'wp_cache_front_page_checks', 'wp_madxcache_cache_list', 'wp_cache_clear_on_post_edit', 'wp_cache_make_known_anon', 'wp_cache_refresh_single_only', 'cache_compression' );
	...
	if( isset( $_POST[ 'action' ] ) && $_POST[ 'action' ] == 'scupdates' ) {
		...
		if( isset( $_POST[ 'wp_madx_cache_late_init' ] ) ) {
			$wp_madx_cache_late_init = 1;
		} else {
			$wp_madx_cache_late_init = 0;
		}
		wp_cache_replace_line('^ *\$wp_madx_cache_late_init', "\$wp_madx_cache_late_init = " . $wp_madx_cache_late_init . ";", $wp_cache_config_file);
		...
		if( isset( $_POST[ 'wp_madxcache_304' ] ) ) {
			$wp_madxcache_304 = 1;
		} else {
			$wp_madxcache_304 = 0;
		}
		wp_cache_replace_line('^ *\$wp_madxcache_304', "\$wp_madxcache_304 = " . $wp_madxcache_304 . ";", $wp_cache_config_file);
		...
		if( isset( $_POST[ 'wp_madxcache_cache_list' ] ) ) {
			$wp_madxcache_cache_list = 1;
		} else {
			$wp_madxcache_cache_list = 0;
		}
		wp_cache_replace_line('^ *\$wp_madxcache_cache_list', "\$wp_madxcache_cache_list = " . $wp_madxcache_cache_list . ";", $wp_cache_config_file);

		if ( isset( $_POST[ 'wp_cache_enabled' ] ) ) {
			wp_cache_enable();
			if ( ! defined( 'DISABLE_madxCACHE' ) ) {
				wp_cache_debug( 'DISABLE_madxCACHE is not set, madx_cache enabled.' );
				wp_madx_cache_enable();
				$madx_cache_enabled = true;
			}
		} else {
			wp_cache_disable();
			wp_madx_cache_disable();
			$madx_cache_enabled = false;
		}
		...
		if ( isset( $_POST['wp_cache_not_logged_in'] ) && $_POST['wp_cache_not_logged_in'] != 0 ) {
			if ( $wp_cache_not_logged_in == 0 && function_exists( 'prune_madx_cache' ) ) {
				prune_madx_cache( $cache_path, true );
			}
		...
		if( isset( $_POST[ 'wp_cache_make_known_anon' ] ) ) {
			if( $wp_cache_make_known_anon == 0 && function_exists( 'prune_madx_cache' ) )
				prune_madx_cache ($cache_path, true);
		...
		if ( defined( 'WPSC_DISABLE_COMPRESSION' ) ) {
			...
			if ( 1 == ini_get( 'zlib.output_compression' ) || "on" == strtolower( ini_get( 'zlib.output_compression' ) ) ) {
				echo '<div class="notice notice-error">' . __( "<strong>Warning!</strong> You attempted to enable compression but <code>zlib.output_compression</code> is enabled. See #21 in the Troubleshooting section of the readme file.", 'wp-madx-cache' ) . '</div>';
			} elseif ( $new_cache_compression !== (int) $cache_compression ) {
				$cache_compression = $new_cache_compression;
				wp_cache_replace_line( '^ *\$cache_compression', "\$cache_compression = $cache_compression;", $wp_cache_config_file );
				if ( function_exists( 'prune_madx_cache' ) ) {
					prune_madx_cache( $cache_path, true );
				}
				delete_option( 'madx_cache_meta' );
			}
		}
	}
}
if ( isset( $_GET[ 'page' ] ) && $_GET[ 'page' ] == 'wpmadxcache' )
	add_action( 'admin_init', 'wp_cache_manager_updates' );

function wp_cache_manager() {
	global $wp_cache_config_file, $valid_nonce, $madxcachedir, $cache_path, $cache_enabled, $cache_compression, $madx_cache_enabled;
	...
	global $wp_cache_not_logged_in, $wp_cache_make_known_anon, $wp_madxcache_cache_list, $cache_page_secret;
	global $wp_madx_cache_front_page_check, $wp_cache_refresh_single_only, $wp_cache_mobile_prefixes;
	global $wp_cache_mod_rewrite, $wp_madxcache_304, $wp_madx_cache_late_init, $wp_cache_front_page_checks, $wp_cache_disable_utf8, $wp_cache_mfunc_enabled;
	global $wp_madx_cache_comments, $wp_cache_home_path, $wpsc_save_headers, $is_nginx;
	...
	if ( function_exists( 'do_cacheaction' ) ) {
		$wp_cache_mobile_browsers = do_cacheaction( 'wp_madx_cache_mobile_browsers', $wp_cache_mobile_browsers );
		$wp_cache_mobile_prefixes = do_cacheaction( 'wp_madx_cache_mobile_prefixes', $wp_cache_mobile_prefixes );
	}
	...
	if ( false == apply_filters( 'wp_madx_cache_error_checking', true ) )
		return false;

	if ( function_exists( 'get_madxcache_dir' ) )
		$madxcachedir = get_madxcache_dir();
...
<?php
	echo '<a name="top"></a>';
	echo '<div class="wrap">';
	echo '<h3>' . __( 'WP madx Cache Settings', 'wp-madx-cache' ) . '</h3>';

	// Set a default.
	if ( false === $cache_enabled && ! isset( $wp_cache_mod_rewrite ) ) {
		$wp_cache_mod_rewrite = 0;
	} elseif ( ! isset( $wp_cache_mod_rewrite ) && $cache_enabled && $madx_cache_enabled ) {
		$wp_cache_mod_rewrite = 1;
	}

	$admin_url = admin_url( 'options-general.php?page=wpmadxcache' );
	$curr_tab  = ! empty( $_GET['tab'] ) ? sanitize_text_field( stripslashes( $_GET['tab'] ) ) : ''; // WPCS: sanitization ok.
	if ( empty( $curr_tab ) ) {
		$curr_tab = 'easy';
		if ( $wp_cache_mod_rewrite ) {
			$curr_tab = 'settings';
			echo '<div class="notice notice-info is-dismissible"><p>' .  __( 'Notice: <em>Expert mode caching enabled</em>. Showing Advanced Settings Page by default.', 'wp-madx-cache' ) . '</p></div>';
		}
	}

	if ( 'preload' === $curr_tab ) {
		if ( true == $madx_cache_enabled && ! defined( 'DISABLEmadxCACHEPRELOADING' ) ) {
			...
			if ( is_array( $preload_counter ) && $preload_counter['c'] > 0 ) {
				$msg .= '<p>' . sprintf( esc_html__( 'Currently caching from post %d to %d.', 'wp-madx-cache' ), ( $preload_counter['c'] - 100 ), $preload_counter['c'] ) . '</p>';
				...
				if ( @file_exists( $cache_path . 'preload_permalink.txt' ) ) {
					...
					$msg .= '<p>' . sprintf( __( '<strong>Page last cached:</strong> %s', 'wp-madx-cache' ), '<span id="preload_status">' . $url . '</span>' ) . '</p>';
				}
				if ( $msg != '' ) {
					echo '<div class="notice notice-warning"><h4>' . esc_html__( 'Preload Active', 'wp-madx-cache' ) . '</h4>' . $msg;
					...
					echo '<input type="hidden" name="page" value="wpmadxcache" />';
					echo '<p><input class="button-primary" type="submit" name="preload_off" value="' . esc_html__( 'Cancel Cache Preload', 'wp-madx-cache' ) . '" /></p>';
					wp_nonce_field( 'wp-cache' );
					echo '</form>';
					echo '</div>';
				}
			}
			next_preload_message( 'wp_cache_preload_hook', __( 'Refresh of cache in %d hours %d minutes and %d seconds.', 'wp-madx-cache' ), 60 );
			next_preload_message( 'wp_cache_full_preload_hook', __( 'Full refresh of cache in %d hours %d minutes and %d seconds.', 'wp-madx-cache' ) );
	...
	if ( isset( $wp_madx_cache_front_page_check ) && $wp_madx_cache_front_page_check == 1 && ! wp_next_scheduled( 'wp_cache_check_site_hook' ) ) {
	...
	if ( isset( $_REQUEST['wp_restore_config'] ) && $valid_nonce ) {
		unlink( $wp_cache_config_file );
		echo '<strong>' . esc_html__( 'Configuration file changed, some values might be wrong. Load the page again from the "Settings" menu to reset them.', 'wp-madx-cache' ) . '</strong>';
	}
	...
	switch ( $curr_tab ) {
		...
		case 'plugins':
			...
			break;
		case 'debug':
			global $wp_madx_cache_debug, $wp_cache_debug_log, $wp_cache_debug_ip, $wp_cache_debug_ip;
			global $wp_madx_cache_front_page_text, $wp_madx_cache_front_page_notification;
			global $wp_madx_cache_advanced_debug, $wp_cache_debug_username, $wp_madx_cache_front_page_clear;
			wpsc_render_partial(
				'debug',
				compact( 'wp_madx_cache_debug', 'wp_cache_debug_log', 'wp_cache_debug_ip', 'cache_path', 'valid_nonce', 'wp_cache_config_file', 'wp_madx_cache_comments', 'wp_madx_cache_front_page_check', 'wp_madx_cache_front_page_clear', 'wp_madx_cache_front_page_text', 'wp_madx_cache_front_page_notification', 'wp_madx_cache_advanced_debug', 'wp_cache_debug_username' )
			);
			break;
		case 'settings':
			global $cache_acceptable_files, $wpsc_rejected_cookies, $cache_rejected_uri, $wp_cache_pages;
			global $cache_max_time, $wp_cache_config_file, $valid_nonce, $madx_cache_enabled, $cache_schedule_type, $cache_scheduled_time, $cache_schedule_interval, $cache_time_interval, $cache_gc_email_me, $wp_cache_preload_on;
			...
			wpsc_render_partial(
				...
				compact(
					...
					'wp_madxcache_cache_list',
					'wp_cache_mutex_disabled',
					'wp_madx_cache_late_init',
					'cache_page_secret',
					'cache_path',
		...
		default:
			wpsc_render_partial(
				...
				array(
					...
					'wp_madx_cache_comments' => $wp_madx_cache_comments,
				)
			);
			break;
	}
	...
	<h4><?php _e( 'Other Site Tools', 'wp-madx-cache' ); ?></h4>
	<ul style="list-style: square; margin-left: 2em;">

	<li><a href="<?php echo esc_url( $wpsc_promo_links['boost'] ); ?>"><?php esc_html_e( 'Boost your page speed scores', 'wp-madx-cache' ); ?></a></li>
	<li><a href="<?php echo esc_url( $wpsc_promo_links['photon'] ); ?>"><?php esc_html_e( 'Speed up images and photos (free)', 'wp-madx-cache' ); ?></a></li>
	<li><a href="<?php echo esc_url( $wpsc_promo_links['videopress'] ); ?>"><?php esc_html_e( 'Fast video hosting (paid)', 'wp-madx-cache' ); ?></a></li>
	<li><a href="<?php echo esc_url( $wpsc_promo_links['crowdsignal'] ); ?>"><?php esc_html_e( 'Add Surveys and Polls to your site', 'wp-madx-cache' ); ?></a></li>
	</ul>
	<h4><?php _e( 'Need Help?', 'wp-madx-cache' ); ?></h4>
	<ol>
	<li><?php printf( __( 'Use the <a href="%1$s">Debug tab</a> for diagnostics.', 'wp-madx-cache' ), admin_url( 'options-general.php?page=wpmadxcache&tab=debug' ) ); ?></li>
	<li><?php printf( __( 'Check out the <a href="%1$s">support forum</a> and <a href="%2$s">FAQ</a>.', 'wp-madx-cache' ), 'https://wordpress.org/support/plugin/wp-madx-cache', 'https://wordpress.org/plugins/wp-madx-cache/#faq' ); ?></li>
	<li><?php printf( __( 'Visit the <a href="%1$s">plugin homepage</a>.', 'wp-madx-cache' ), 'https://wordpress.org/plugins/wp-madx-cache/' ); ?></li>
	<li><?php printf( __( 'Try out the <a href="%1$s">development version</a> for the latest fixes (<a href="%2$s">changelog</a>).', 'wp-madx-cache' ), 'https://odd.blog/y/2o', 'https://plugins.trac.wordpress.org/log/wp-madx-cache/' ); ?></li>
	</ol>
	<h4><?php esc_html_e( 'Rate This Plugin', 'wp-madx-cache' ); ?></h4>
	<p><?php printf( __( 'Please <a href="%s">rate us</a> and give feedback.', 'wp-madx-cache' ), 'https://wordpress.org/support/plugin/wp-madx-cache/reviews?rate=5#new-post' ); ?></p>

	<?php
	if ( isset( $wp_madxcache_cache_list ) && $wp_madxcache_cache_list ) {
		$start_date = get_option( 'wpmadxcache_start' );
		if ( ! $start_date ) {
			$start_date = time();
		}
		?>
		<p><?php printf( __( 'Cached pages since %1$s : <strong>%2$s</strong>', 'wp-madx-cache' ), date( 'M j, Y', $start_date ), number_format( get_option( 'wpmadxcache_count' ) ) ); ?></p>
		<p><?php _e( 'Newest Cached Pages:', 'wp-madx-cache' ); ?><ol>
			<?php
			foreach ( array_reverse( (array) get_option( 'madxcache_last_cached' ) ) as $url ) {
				$since = time() - strtotime( $url['date'] );
				echo "<li><a title='" . sprintf( esc_html__( 'Cached %s seconds ago', 'wp-madx-cache' ), (int) $since ) . "' href='" . site_url( $url['url'] ) . "'>" . substr( $url['url'], 0, 20 ) . "</a></li>\n";
			}
			?>
			</ol>
			<small><?php esc_html_e( '(may not always be accurate on busy sites)', 'wp-madx-cache' ); ?></small>
		</p><?php
	} elseif ( false == get_option( 'wpmadxcache_start' ) ) {
			update_option( 'wpmadxcache_start', time() );
			update_option( 'wpmadxcache_count', 0 );
	}
	?>
	</div>
	</td></table>
...
function wpsc_plugins_tab() {
	echo '<p>' . esc_html__( 'Cache plugins are PHP scripts you\'ll find in a dedicated folder inside the WP madx Cache folder (wp-madx-cache/plugins/). They load at the same time as WP madx Cache, and before regular WordPress plugins.', 'wp-madx-cache' ) . '</p>';
	echo '<p>' . esc_html__( 'Keep in mind that cache plugins are for advanced users only. To create and manage them, you\'ll need extensive knowledge of both PHP and WordPress actions.', 'wp-madx-cache' ) . '</p>';
	echo '<p>' . sprintf( __( '<strong>Warning</strong>! Due to the way WordPress upgrades plugins, the ones you upload to the WP madx Cache folder (wp-madx-cache/plugins/) will be deleted when you upgrade WP madx Cache. To avoid this loss, load your cache plugins from a different location. When you set <strong>$wp_cache_plugins_dir</strong> to the new location in wp-config.php, WP madx Cache will look there instead. <br />You can find additional details in the <a href="%s">developer documentation</a>.', 'wp-madx-cache' ), 'https://odd.blog/wp-madx-cache-developers/' ) . '</p>';
	ob_start();
	if ( defined( 'WP_CACHE' ) ) {
		if ( function_exists( 'do_cacheaction' ) ) {
	...
	if ( SUBMITDISABLED == ' ' && $out != '' ) {
		echo '<h4>' . esc_html__( 'Available Plugins', 'wp-madx-cache' ) . '</h4>';
		echo '<ol>';
		echo $out;
		echo '</ol>';
	}
...
function wpsc_admin_tabs( $current = '' ) {
	global $cache_enabled, $madx_cache_enabled, $wp_cache_mod_rewrite;
	...
	$admin_url  = admin_url( 'options-general.php?page=wpmadxcache' );
	$admin_tabs = array(
		'easy'     => __( 'Easy', 'wp-madx-cache' ),
		'settings' => __( 'Advanced', 'wp-madx-cache' ),
		'cdn'      => __( 'CDN', 'wp-madx-cache' ),
		'contents' => __( 'Contents', 'wp-madx-cache' ),
		'preload'  => __( 'Preload', 'wp-madx-cache' ),
		'plugins'  => __( 'Plugins', 'wp-madx-cache' ),
		'debug'    => __( 'Debug', 'wp-madx-cache' ),
	);
...
function comment_form_lockdown_message() {
	?><p><?php _e( "Comment moderation is enabled. Your comment may take some time to appear.", 'wp-madx-cache' ); ?></p><?php
}
...
function wp_update_lock_down() {
	...
	if ( isset( $_POST[ 'wp_lock_down' ] ) && $valid_nonce ) {
		...
		if ( $wp_lock_down == '0' && function_exists( 'prune_madx_cache' ) )
			prune_madx_cache( $cache_path, true ); // clear the cache after lockdown
		return $wp_lock_down;
	}
...
function wpsc_update_direct_pages() {
	...
	if ( $valid_nonce && array_key_exists('new_direct_page', $_POST) && $_POST[ 'new_direct_page' ] && '' != $_POST[ 'new_direct_page' ] ) {
		...
		if ( $page != '/' || false == is_array( $cached_direct_pages ) || in_array( $page, $cached_direct_pages ) == false ) {
			...
			wpsc_delete_files( get_madxcache_dir() . $page );
		}
	}
	...
	if ( $valid_nonce && array_key_exists('deletepage', $_POST) && $_POST[ 'deletepage' ] ) {
		...
		if ( substr( $pagefile, 0, strlen( ABSPATH ) ) != ABSPATH || false == wp_cache_confirm_delete( ABSPATH . $page ) ) {
			die( __( 'Cannot delete directory', 'wp-madx-cache' ) );
		}
...
function wpsc_lockdown() {
	global $cached_direct_pages, $cache_enabled, $madx_cache_enabled;
	...
	wpsc_render_partial(
		'lockdown',
		compact( 'cached_direct_pages', 'cache_enabled', 'madx_cache_enabled', 'admin_url', 'wp_lock_down' )
	);
}
...
function wpsc_update_debug_settings() {
	global $wp_madx_cache_debug, $wp_cache_debug_log, $wp_cache_debug_ip, $cache_path, $valid_nonce, $wp_cache_config_file, $wp_madx_cache_comments;
	global $wp_madx_cache_front_page_check, $wp_madx_cache_front_page_clear, $wp_madx_cache_front_page_text, $wp_madx_cache_front_page_notification, $wp_madx_cache_advanced_debug;
	global $wp_cache_debug_username;

	if ( ! isset( $wp_madx_cache_comments ) ) {
		$wp_madx_cache_comments = 1; // defaults to "enabled".
		wp_cache_setting( 'wp_madx_cache_comments', $wp_madx_cache_comments );
	}

	if ( false == $valid_nonce ) {
		return array (
			'wp_madx_cache_debug' => $wp_madx_cache_debug,
			'wp_cache_debug_log' => $wp_cache_debug_log,
			'wp_cache_debug_ip' => $wp_cache_debug_ip,
			'wp_madx_cache_comments' => $wp_madx_cache_comments,
			'wp_madx_cache_front_page_check' => $wp_madx_cache_front_page_check,
			'wp_madx_cache_front_page_clear' => $wp_madx_cache_front_page_clear,
			'wp_madx_cache_front_page_text' => $wp_madx_cache_front_page_text,
			'wp_madx_cache_front_page_notification' => $wp_madx_cache_front_page_notification,
			'wp_madx_cache_advanced_debug' => $wp_madx_cache_advanced_debug,
	...
	$wp_madx_cache_debug = ( isset( $_POST[ 'wp_madx_cache_debug' ] ) && $_POST[ 'wp_madx_cache_debug' ] == 1 ) ? 1 : 0;
	wp_cache_setting( 'wp_madx_cache_debug', $wp_madx_cache_debug );
	...
	if ( isset( $_POST[ 'wp_cache_debug' ] ) ) {
		...
		$wp_madx_cache_comments = isset( $_POST[ 'wp_madx_cache_comments' ] ) ? 1 : 0;
		wp_cache_setting( 'wp_madx_cache_comments', $wp_madx_cache_comments );
		...
		$wp_madx_cache_front_page_check = isset( $_POST[ 'wp_madx_cache_front_page_check' ] ) ? 1 : 0;
		wp_cache_setting( 'wp_madx_cache_front_page_check', $wp_madx_cache_front_page_check );
		$wp_madx_cache_front_page_clear = isset( $_POST[ 'wp_madx_cache_front_page_clear' ] ) ? 1 : 0;
		wp_cache_setting( 'wp_madx_cache_front_page_clear', $wp_madx_cache_front_page_clear );
		if ( isset( $_POST[ 'wp_madx_cache_front_page_text' ] ) ) {
			$wp_madx_cache_front_page_text = esc_html( preg_replace( '/[ <>\'\"\r\n\t\(\)\$\[\];#]/', '', $_POST[ 'wp_madx_cache_front_page_text' ] ) );
		} else {
			$wp_madx_cache_front_page_text = '';
		}
		wp_cache_setting( 'wp_madx_cache_front_page_text', $wp_madx_cache_front_page_text );
		$wp_madx_cache_front_page_notification = isset( $_POST[ 'wp_madx_cache_front_page_notification' ] ) ? 1 : 0;
		wp_cache_setting( 'wp_madx_cache_front_page_notification', $wp_madx_cache_front_page_notification );
		if ( $wp_madx_cache_front_page_check == 1 && !wp_next_scheduled( 'wp_cache_check_site_hook' ) ) {
			wp_schedule_single_event( time() + 360 , 'wp_cache_check_site_hook' );
...
function wp_madx_cache_enable() {
	global $madxcachedir, $wp_cache_config_file, $madx_cache_enabled;

	if ( $madx_cache_enabled ) {
		wp_cache_debug( 'wp_madx_cache_enable: already enabled' );
		return true;
	}

	wp_cache_setting( 'madx_cache_enabled', true );
	wp_cache_debug( 'wp_madx_cache_enable: enable cache' );

	$madx_cache_enabled = true;

	if ( is_dir( $madxcachedir . '.disabled' ) ) {
		if ( is_dir( $madxcachedir ) ) {
			prune_madx_cache( $madxcachedir . '.disabled', true );
			@unlink( $madxcachedir . '.disabled' );
		} else {
			@rename( $madxcachedir . '.disabled', $madxcachedir );
		}
	}
}

function wp_madx_cache_disable() {
	global $cache_path, $madxcachedir, $wp_cache_config_file, $madx_cache_enabled;

	if ( ! $madx_cache_enabled ) {
		wp_cache_debug( 'wp_madx_cache_disable: already disabled' );
		return true;
	}

	wp_cache_setting( 'madx_cache_enabled', false );
	wp_cache_debug( 'wp_madx_cache_disable: disable cache' );

	$madx_cache_enabled = false;

	if ( is_dir( $madxcachedir ) ) {
		@rename( $madxcachedir, $madxcachedir . '.disabled' );
	}
	sleep( 1 ); // allow existing processes to write to the madxcachedir and then delete it
	if ( function_exists( 'prune_madx_cache' ) && is_dir( $madxcachedir ) ) {
		prune_madx_cache( $cache_path, true );
	}
...
function wp_cache_is_enabled() {
	...
	if ( get_option( 'gzipcompression' ) ) {
		echo '<strong>' . __( 'Warning', 'wp-madx-cache' ) . '</strong>: ' . __( 'GZIP compression is enabled in WordPress, wp-cache will be bypassed until you disable gzip compression.', 'wp-madx-cache' );
		return false;
	}
...
function wp_cache_remove_index() {
	...
	@unlink( $cache_path . "madxcache/index.html" );
...
function wp_cache_index_notice() {
	...
	if ( false == wpmadxcache_site_admin() )
		return false;
	if ( false == get_site_option( 'wp_madx_cache_index_detected' ) )
		return false;
	...
	if ( get_site_option( 'wp_madx_cache_index_detected' ) == 2 ) {
		update_site_option( 'wp_madx_cache_index_detected', 3 );
		echo "<div class='error' style='padding: 10px 10px 50px 10px'>";
		echo "<h2>" . __( 'WP madx Cache Warning!', 'wp-madx-cache' ) . '</h2>';
		echo '<p>' . __( 'All users of this site have been logged out to refresh their login cookies.', 'wp-madx-cache' ) . '</p>';
		echo '</div>';
		return false;
	} elseif ( get_site_option( 'wp_madx_cache_index_detected' ) != 3 ) {
		echo "<div id='wpsc-index-warning' class='error notice' style='padding: 10px 10px 50px 10px'>";
		echo "<h2>" . __( 'WP madx Cache Warning!', 'wp-madx-cache' ) . '</h2>';
		echo '<p>' . __( 'Your server is configured to show files and directories, which may expose sensitive data such as login cookies to attackers in the cache directories. That has been fixed by adding a file named index.html to each directory. If you use simple caching, consider moving the location of the cache directory on the Advanced Settings page.', 'wp-madx-cache' ) . '</p>';
		echo "<p><strong>";
		_e( 'If you just installed WP madx Cache for the first time, you can dismiss this message. Otherwise, you should probably refresh the login cookies of all logged in WordPress users here by clicking the logout link below.', 'wp-madx-cache' );
		echo "</strong></p>";
		if ( -1 == version_compare( $wp_version, '4.0' ) ) {
			echo '<p>' . __( 'Your site is using a very old version of WordPress. When you update to the latest version everyone will be logged out and cookie information updated.', 'wp-madx-cache' ) . '</p>';
		} else {
			echo '<p>' . __( 'The logout link will log out all WordPress users on this site except you. Your authentication cookie will be updated, but you will not be logged out.', 'wp-madx-cache' ) . '</p>';
		}
		echo "<a id='wpsc-dismiss' href='#'>" . __( 'Dismiss', 'wp-madx-cache' ) . "</a>";
		if ( 1 == version_compare( $wp_version, '4.0' ) ) {
			echo "	| <a href='" . wp_nonce_url( admin_url( '?action=wpsclogout' ), 'wpsc_logout' ) . "'>" . __( 'Logout', 'wp-madx-cache' ) . "</a>";
		}
		echo "</div>";
?>
		<script  type='text/javascript'>
...
function wp_cache_logout_all() {
	...
	if ( isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] == 'wpsclogout' && wp_verify_nonce( $_GET[ '_wpnonce' ], 'wpsc_logout' ) ) {
		...
		update_site_option( 'wp_madx_cache_index_detected', 2 );
...
function wp_cache_add_index_protection() {
	...
	if ( is_dir( $cache_path ) && false == is_file( "$cache_path/index.html" ) ) {
		...
		if ( false == is_wp_error( $page ) ) {
			if ( false == get_site_option( 'wp_madx_cache_index_detected' )
				...
				add_site_option( 'wp_madx_cache_index_detected', 1 ); // only show this once
			}
		}
	...
	$directories = array( $cache_path, $cache_path . '/madxcache/', $cache_path . '/blogs/', $blog_cache_dir, $blog_cache_dir . "/meta" );
...
function wp_cache_add_site_cache_index() {
	...
	wp_cache_add_index_protection(); // root and madxcache
...
function wp_cache_verify_cache_dir() {
	...
	if ( !file_exists($cache_path) ) {
		if ( !is_writeable_ACLSafe( $dir ) || !($dir = mkdir( $cache_path ) ) ) {
				echo "<strong>" . __( 'Error', 'wp-madx-cache' ) . ":</strong> " . sprintf( __( 'Your cache directory (<strong>%1$s</strong>) did not exist and couldn&#8217;t be created by the web server. Check %1$s permissions.', 'wp-madx-cache' ), $dir );
				return false;
		}
	}
	if ( !is_writeable_ACLSafe($cache_path)) {
		echo "<strong>" . __( 'Error', 'wp-madx-cache' ) . ":</strong> " . sprintf( __( 'Your cache directory (<strong>%1$s</strong>) or <strong>%2$s</strong> need to be writable for this plugin to work. Double-check it.', 'wp-madx-cache' ), $cache_path, $dir );
		return false;
	}
	...
	if ( file_exists($wp_cache_config_file) ) {
		...
		if( strpos( $lines, 'WPCACHEHOME' ) === false ) {
			if( is_writeable_ACLSafe( $wp_cache_config_file ) ) {
				...
				echo "<strong>" . __( 'Error', 'wp-madx-cache' ) . ":</strong> " . sprintf( __( 'Your WP-Cache config file (<strong>%s</strong>) is out of date and not writable by the Web server. Please delete it and refresh this page.', 'wp-madx-cache' ), $wp_cache_config_file );
				return false;
			}
		}
	} elseif( !is_writeable_ACLSafe($dir)) {
		echo "<strong>" . __( 'Error', 'wp-madx-cache' ) . ":</strong> " . sprintf( __( 'Configuration file missing and %1$s  directory (<strong>%2$s</strong>) is not writable by the web server. Check its permissions.', 'wp-madx-cache' ), WP_CONTENT_DIR, $dir );
		return false;
	}

	if ( !file_exists($wp_cache_config_file) ) {
		if ( !file_exists($wp_cache_config_file_sample) ) {
			echo "<strong>" . __( 'Error', 'wp-madx-cache' ) . ":</strong> " . sprintf( __( 'Sample WP-Cache config file (<strong>%s</strong>) does not exist. Verify your installation.', 'wp-madx-cache' ), $wp_cache_config_file_sample );
			return false;
		}
		...
		if ( is_file( __DIR__ . '/wp-cache-config-sample.php' ) ) {
			...
			wp_cache_replace_line('define\(\ \'WPCACHEHOME', "\tdefine( 'WPCACHEHOME', WP_CONTENT_DIR . \"{$dir}/wp-madx-cache/\" );", $wp_cache_config_file);
		}
		$new = true;
	}
...
function wp_cache_create_advanced_cache() {
	...
	if ( ! apply_filters( 'wpsc_enable_wp_config_edit', true ) ) {
		echo '<div class="notice notice-error"><h4>' . __( 'Warning', 'wp-madx-cache' ) . "! " . sprintf( __( 'Not allowed to edit %s per configuration.', 'wp-madx-cache' ), $global_config_file ) . "</h4></div>";
		return false;
	}
	...
	if (
		...
		if (
			...
			echo '<div class="notice notice-error"><h4>' . __( 'Warning', 'wp-madx-cache' ) . "! <em>" . sprintf( __( 'Could not update %s!</em> WPCACHEHOME must be set in config file.', 'wp-madx-cache' ), $global_config_file ) . "</h4></div>";
			return false;
		}
	}
	$ret = true;
...
function wpsc_check_advanced_cache() {
	...
	if ( file_exists( $wpsc_advanced_cache_filename ) ) {
		$file = file_get_contents( $wpsc_advanced_cache_filename );
		if ( strpos( $file, "WP madx CACHE 0.8.9.1" ) || strpos( $file, "WP madx CACHE 1.2" ) ) {
			return true;
		} else {
	...
	if ( false == $ret ) {
		if ( $other_advanced_cache ) {
			echo '<div style="width: 50%" class="notice notice-error"><h2>' . __( 'Warning! You may not be allowed to use this plugin on your site.', 'wp-madx-cache' ) . "</h2>";
			echo '<p>' .
				sprintf(
					__( 'The file %s was created by another plugin or by your system administrator. Please examine the file carefully by FTP or SSH and consult your hosting documentation. ', 'wp-madx-cache' ),
					$wpsc_advanced_cache_filename
				) .
				'</p>';
			echo '<p>' .
				__( 'If it was created by another caching plugin please uninstall that plugin first before activating WP madx Cache. If the file is not removed by that action you should delete the file manually.', 'wp-madx-cache' ),
				'</p>';
			echo '<p><strong>' .
				__( 'If you need support for this problem contact your hosting provider.', 'wp-madx-cache' ),
				'</strong></p>';
		} elseif ( ! is_writeable_ACLSafe( $wpsc_advanced_cache_filename ) ) {
			echo '<div class="notice notice-error"><h2>' . __( 'Warning', 'wp-madx-cache' ) . "! <em>" . sprintf( __( '%s/advanced-cache.php</em> cannot be updated.', 'wp-madx-cache' ), WP_CONTENT_DIR ) . "</h2>";
			echo '<ol>';
			echo "<li>" .
				sprintf(
					__( 'Make %1$s writable using the chmod command through your ftp or server software. (<em>chmod 777 %1$s</em>) and refresh this page. This is only a temporary measure and you&#8217;ll have to make it read only afterwards again. (Change 777 to 755 in the previous command)', 'wp-madx-cache' ),
					WP_CONTENT_DIR
				) .
				"</li>";
			echo "<li>" . sprintf( __( 'Refresh this page to update <em>%s/advanced-cache.php</em>', 'wp-madx-cache' ), WP_CONTENT_DIR ) . "</li></ol>";
			echo sprintf( __( 'If that doesn&#8217;t work, make sure the file <em>%s/advanced-cache.php</em> doesn&#8217;t exist:', 'wp-madx-cache' ), WP_CONTENT_DIR ) . "<ol>";
			echo "</ol>";
		}
		echo "</div>";
		return false;
...
function wp_cache_check_global_config() {
	...
	if (
		...
		if ( defined( 'WP_CACHE' ) && constant( 'WP_CACHE' ) == false ) {
			echo '<div class="notice notice-error">' . __( "<h4>WP_CACHE constant set to false</h4><p>The WP_CACHE constant is used by WordPress to load the code that serves cached pages. Unfortunately, it is set to false. Please edit your wp-config.php and add or edit the following line above the final require_once command:<br /><br /><code>define('WP_CACHE', true);</code></p>", 'wp-madx-cache' ) . "</div>";
		} else {
			echo '<div class="notice notice-error"><p>' . __( "<strong>Error: WP_CACHE is not enabled</strong> in your <code>wp-config.php</code> file and I couldn&#8217;t modify it.", 'wp-madx-cache' ) . "</p>";
			echo "<p>" . sprintf( __( "Edit <code>%s</code> and add the following line:<br /> <code>define('WP_CACHE', true);</code><br />Otherwise, <strong>WP-Cache will not be executed</strong> by WordPress core. ", 'wp-madx-cache' ), $global_config_file ) . "</p></div>";
		}
		return false;
	}  else {
		echo "<div class='notice notice-warning'>" . __( '<h4>WP_CACHE constant added to wp-config.php</h4><p>If you continue to see this warning message please see point 5 of the <a href="https://wordpress.org/plugins/wp-madx-cache/faq/">Troubleshooting Guide</a>. The WP_CACHE line must be moved up.', 'wp-madx-cache' ) . "</p></div>";
	}
	return true;
}
...
function wpsc_generate_sizes_array() {
	$sizes = array();
	$cache_types  = apply_filters( 'wpsc_cache_types', array( 'madxcache', 'wpcache' ) );
...
function wp_cache_regenerate_cache_file_stats() {
	global $cache_compression, $madxcachedir, $file_prefix, $wp_cache_preload_on, $cache_max_time;
	...
	$now = time();
	if (is_dir( $madxcachedir ) ) {
		if ( $dh = opendir( $madxcachedir ) ) {
			while ( ( $entry = readdir( $dh ) ) !== false ) {
				if ( $entry != '.' && $entry != '..' ) {
					$sizes = wpsc_dirsize( trailingslashit( $madxcachedir ) . $entry, $sizes );
				}
			}
			closedir( $dh );
		}
	...
	if ( $cache_compression ) {
		$sizes[ 'madxcache' ][ 'cached' ]  = intval( $sizes[ 'madxcache' ][ 'cached' ] / 2 );
		$sizes[ 'madxcache' ][ 'expired' ] = intval( $sizes[ 'madxcache' ][ 'expired' ] / 2 );
	}
	$cache_stats = array( 'generated' => time(), 'madxcache' => $sizes[ 'madxcache' ], 'wpcache' => $sizes[ 'wpcache' ] );
	update_option( 'madxcache_stats', $cache_stats );
	return $cache_stats;
}

function wp_cache_files() {
	global $cache_path, $file_prefix, $cache_max_time, $valid_nonce, $madxcachedir, $madx_cache_enabled, $blog_cache_dir, $cache_compression;
	...
	echo '<fieldset class="options" id="show-this-fieldset"><h4>' . __( 'Cache Contents', 'wp-madx-cache' ) . '</h4>';

	$cache_stats = get_option( 'madxcache_stats' );
	...
	if ( ( $handle = @opendir( $blog_cache_dir ) ) ) {
		...
		if ( $valid_nonce && isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] == 'deletemadxcache' ) {
			$madxcacheuri = wpsc_deep_replace( array( '..', '\\', 'index.php' ), preg_replace( '/[ <>\'\"\r\n\t\(\)]/', '', preg_replace("/(\?.*)?$/", '', base64_decode( $_GET[ 'uri' ] ) ) ) );
			$madxcacheuri = trailingslashit( realpath( $cache_path . 'madxcache/' . $madxcacheuri ) );
			if ( wp_cache_confirm_delete( $madxcacheuri ) ) {
				printf( __( "Deleting madxcache file: <strong>%s</strong><br />", 'wp-madx-cache' ), $madxcacheuri );
				wpsc_delete_files( $madxcacheuri );
				prune_madx_cache( $madxcacheuri . 'page', true );
				@rmdir( $madxcacheuri );
			} else {
				wp_die( __( 'Warning! You are not allowed to delete that file', 'wp-madx-cache' ) );
			}
		}
			...
			if ( strpos( $file, $file_prefix ) !== false && substr( $file, -4 ) == '.php' ) {
				...
				if ( $valid_nonce && isset( $_GET[ 'listfiles' ] ) ) {
					...
					if ( $deleteuri != '' && $meta[ 'uri' ] == $deleteuri ) {
						printf( __( "Deleting wp-cache file: <strong>%s</strong><br />", 'wp-madx-cache' ), esc_html( $deleteuri ) );
	...
	$cache_stats = wp_cache_regenerate_cache_file_stats();
		...
		echo "<p>" . __( 'Cache stats are not automatically generated. You must click the link below to regenerate the stats on this page.', 'wp-madx-cache' ) . "</p>";
		echo "<a href='" . wp_nonce_url( add_query_arg( array( 'page' => 'wpmadxcache', 'tab' => 'contents', 'action' => 'regenerate_cache_stats' ) ), 'wp-cache' ) . "'>" . __( 'Regenerate cache stats', 'wp-madx-cache' ) . "</a>";
		if ( is_array( $cache_stats ) ) {
			echo "<p>" . sprintf( __( 'Cache stats last generated: %s minutes ago.', 'wp-madx-cache' ), number_format( ( time() - $cache_stats[ 'generated' ] ) / 60 ) ) . "</p>";
		}
		$cache_stats = get_option( 'madxcache_stats' );
	...
	if ( is_array( $cache_stats ) ) {
		...
		echo "<p><strong>" . __( 'WP-Cache', 'wp-madx-cache' ) . " ({$fsize})</strong></p>";
		echo "<ul><li>" . sprintf( __( '%s Cached Pages', 'wp-madx-cache' ), $cache_stats[ 'wpcache' ][ 'cached' ] ) . "</li>";
		echo "<li>" . sprintf( __( '%s Expired Pages', 'wp-madx-cache' ),    $cache_stats[ 'wpcache' ][ 'expired' ] ) . "</li></ul>";
		if ( array_key_exists('fsize', (array)$cache_stats[ 'madxcache' ]) )
			$fsize = $cache_stats[ 'madxcache' ][ 'fsize' ] / 1024;
		else
			$fsize = 0;
		$fsize = wp_cache_format_fsize( $fsize );
		echo "<p><strong>" . __( 'WP-madx-Cache', 'wp-madx-cache' ) . " ({$fsize})</strong></p>";
		echo "<ul><li>" . sprintf( __( '%s Cached Pages', 'wp-madx-cache' ), $cache_stats[ 'madxcache' ][ 'cached' ] ) . "</li>";
		if ( isset( $now ) && isset( $cache_stats ) )
			$age = intval( ( $now - $cache_stats['generated'] ) / 60 );
		else
			$age = 0;
		echo "<li>" . sprintf( __( '%s Expired Pages', 'wp-madx-cache' ), $cache_stats[ 'madxcache' ][ 'expired' ] ) . "</li></ul>";
		if ( $valid_nonce && array_key_exists('listfiles', $_GET) && isset( $_GET[ 'listfiles' ] ) ) {
			...
			$cache_description = array( 'madxcache' => __( 'WP-madx-Cached', 'wp-madx-cache' ), 'wpcache' => __( 'WP-Cached', 'wp-madx-cache' ) );
			foreach( $cache_stats as $type => $details ) {
				...
				foreach( array( 'cached_list' => 'Fresh', 'expired_list' => 'Stale' ) as $list => $description ) {
					if ( is_array( $details[ $list ] ) & !empty( $details[ $list ] ) ) {
						echo "<h5>" . sprintf( __( '%s %s Files', 'wp-madx-cache' ), $description, $cache_description[ $type ] ) . "</h5>";
						echo "<table class='widefat'><tr><th>#</th><th>" . __( 'URI', 'wp-madx-cache' ) . "</th><th>" . __( 'Files', 'wp-madx-cache' ) . "</th><th>" . __( 'Age', 'wp-madx-cache' ) . "</th><th>" . __( 'Delete', 'wp-madx-cache' ) . "</th></tr>";
						$c = 1;
						$flip = 1;
						...
						foreach( $details[ $list ] as $directory => $d ) {
							...
							echo "<tr $bg><td>$c</td><td> <a href='http://{$directory}'>{$directory}</a></td><td>{$d[ 'files' ]}</td><td>{$age}</td><td><a href='" . wp_nonce_url( add_query_arg( array( 'page' => 'wpmadxcache', 'action' => 'deletemadxcache', 'uri' => base64_encode( $directory ) ) ), 'wp-cache' ) . "#listfiles'>X</a></td></tr>\n";
							$flip = !$flip;
							++$c;
						}
						echo "</table>";
			...
			echo "</div>";
			echo "<p><a href='?page=wpmadxcache&tab=contents#top'>" . __( 'Hide file list', 'wp-madx-cache' ) . "</a></p>";
		} elseif ( $cache_stats[ 'madxcache' ][ 'cached' ] > 500 || $cache_stats[ 'madxcache' ][ 'expired' ] > 500 || $cache_stats[ 'wpcache' ][ 'cached' ] > 500 || $cache_stats[ 'wpcache' ][ 'expired' ] > 500 ) {
			echo "<p><em>" . __( 'Too many cached files, no listing possible.', 'wp-madx-cache' ) . "</em></p>";
		} else {
			echo "<p><a href='" . wp_nonce_url( add_query_arg( array( 'page' => 'wpmadxcache', 'listfiles' => '1' ) ), 'wp-cache' ) . "#listfiles'>" . __( 'List all cached files', 'wp-madx-cache' ) . "</a></p>";
		}
		if ( $cache_max_time > 0 )
			echo "<p>" . sprintf( __( 'Expired files are files older than %s seconds. They are still used by the plugin and are deleted periodically.', 'wp-madx-cache' ), $cache_max_time ) . "</p>";
		if ( $wp_cache_preload_on )
			echo "<p>" . __( 'Preload mode is enabled. madxcache files will never be expired.', 'wp-madx-cache' ) . "</p>";
	} // cache_stats
...
function wp_cache_delete_buttons() {
	...
	echo '<div class="submit" style="float:left"><input class="button-primary" type="submit" ' . SUBMITDISABLED . 'value="' . __( 'Delete Expired', 'wp-madx-cache' ) . '" /></div>';
	wp_nonce_field('wp-cache');
	echo "</form>\n";
	...
	echo '<div class="submit" style="float:left;margin-left:10px"><input id="deletepost" class="button-secondary" type="submit" ' . SUBMITDISABLED . 'value="' . __( 'Delete Cache', 'wp-madx-cache' ) . '" /></div>';
	wp_nonce_field('wp-cache');
	echo "</form>\n";
	if ( is_multisite() && wpmadxcache_site_admin() ) {
		echo '<form name="wp_cache_content_delete" action="' . esc_url_raw( add_query_arg( 'tab', 'contents', $admin_url ) . '#listfiles' ) . '" method="post">';
		echo '<input type="hidden" name="wp_delete_all_cache" />';
		echo '<div class="submit" style="float:left;margin-left:10px"><input id="deleteallpost" class="button-secondary" type="submit" ' . SUBMITDISABLED . 'value="' . __( 'Delete Cache On All Blogs', 'wp-madx-cache' ) . '" /></div>';
		wp_nonce_field('wp-cache');
		echo "</form>\n";
	}
}
...
function delete_cache_dashboard() {
	if ( function_exists( '_deprecated_function' ) ) {
		_deprecated_function( __FUNCTION__, 'WP madx Cache 1.6.4' );
	}
	...
	echo "<li><a href='" . wp_nonce_url( 'options-general.php?page=wpmadxcache&wp_delete_cache=1', 'wp-cache' ) . "' target='_blank' title='" . __( 'Delete madx Cache cached files (opens in new window)', 'wp-madx-cache' ) . "'>" . __( 'Delete Cache', 'wp-madx-cache' ) . "</a></li>";
}
...
function wpsc_dirsize($directory, $sizes) {
	...
	if (is_dir($directory)) {
		...
		if ( strpos( $directory, '/' . $file_prefix ) !== false ) {
			...
			$cache_type = 'madxcache';
		}
		$keep_fresh = false;
		if ( $cache_type === 'madxcache' && $wp_cache_preload_on ) {
			$keep_fresh = true;
		}
		...
		if ( $valid_nonce && isset( $_GET['listfiles'] ) ) {
			$dir = str_replace( $cache_path . 'madxcache/', '', dirname( $directory ) );
			$age = $now - $filem;
...
function wp_cache_clean_cache( $file_prefix, $all = false ) {
	global $cache_path, $madxcachedir, $blog_cache_dir;

	do_action( 'wp_cache_cleared' );

	if ( $all == true && wpmadxcache_site_admin() && function_exists( 'prune_madx_cache' ) ) {
		prune_madx_cache( $cache_path, true );
		return true;
	}
	if ( $madxcachedir == '' )
	...
	if (function_exists ('prune_madx_cache')) {
		if( is_dir( $madxcachedir ) ) {
			prune_madx_cache( $madxcachedir, true );
		} elseif( is_dir( $madxcachedir . '.disabled' ) ) {
			prune_madx_cache( $madxcachedir . '.disabled', true );
		}
		$_POST[ 'madx_cache_stats' ] = 1; // regenerate madx cache stats;
	} else {
		wp_cache_debug( 'Warning! prune_madx_cache() not found in wp-cache.php', 1 );
	}
...
function wp_cache_clean_expired($file_prefix) {
	...
	if(function_exists('wp_cache_phase2_clean_expired')) {
		if ( $wp_cache_preload_on != 1 && function_exists ('prune_madx_cache')) {
			$dir = get_madxcache_dir();
			if( is_dir( $dir ) ) {
				prune_madx_cache( $dir );
			} elseif( is_dir( $dir . '.disabled' ) ) {
				prune_madx_cache( $dir . '.disabled' );
			}
			$_POST[ 'madx_cache_stats' ] = 1; // regenerate madx cache stats;
		}
...
function wp_cache_catch_404() {
	global $wp_cache_404;
	if ( function_exists( '_deprecated_function' ) )
		_deprecated_function( __FUNCTION__, 'WP madx Cache 1.5.6' );
	$wp_cache_404 = false;
	if( is_404() )
		$wp_cache_404 = true;
}
//More info - https://github.com/Automattic/wp-madx-cache/pull/373
...
function wp_cache_favorite_action( $actions ) {
	...
	$actions[ wp_nonce_url( 'options-general.php?page=wpmadxcache&wp_delete_cache=1&tab=contents', 'wp-cache' ) ] = array( __( 'Delete Cache', 'wp-madx-cache' ), 'manage_options' );
...
function wp_cache_plugin_notice( $plugin ) {
	...
	if( $plugin == 'wp-madx-cache/wp-cache.php' && !$cache_enabled && function_exists( 'admin_url' ) )
		echo '<td colspan="5" class="plugin-update">' . sprintf( __( 'WP madx Cache must be configured. Go to <a href="%s">the admin page</a> to enable and configure the plugin.', 'wp-madx-cache' ), admin_url( 'options-general.php?page=wpmadxcache' ) ) . '</td>';
}
add_action( 'after_plugin_row', 'wp_cache_plugin_notice' );

function wp_cache_plugin_actions( $links, $file ) {
	if( $file == 'wp-madx-cache/wp-cache.php' && function_exists( 'admin_url' ) ) {
		$settings_link = '<a href="' . admin_url( 'options-general.php?page=wpmadxcache' ) . '">' . __( 'Settings', 'wp-madx-cache' ) . '</a>';
...
function wp_cache_admin_notice() {
	...
	if( substr( $_SERVER['PHP_SELF'], -11 ) == 'plugins.php' && !$cache_enabled && function_exists( 'admin_url' ) )
		echo '<div class="notice notice-info"><p><strong>' . sprintf( __('WP madx Cache is disabled. Please go to the <a href="%s">plugin admin page</a> to enable caching.', 'wp-madx-cache' ), admin_url( 'options-general.php?page=wpmadxcache' ) ) . '</strong></p></div>';
	...
	if ( defined( 'WP_CACHE' ) && WP_CACHE == true && ( defined( 'ADVANCEDCACHEPROBLEM' ) || ( $cache_enabled && false == isset( $wp_cache_phase1_loaded ) ) ) ) {
		if ( wp_cache_create_advanced_cache() ) {
			echo '<div class="notice notice-error"><p>' . sprintf( __( 'Warning! WP madx Cache caching <strong>was</strong> broken but has been <strong>fixed</strong>! The script advanced-cache.php could not load wp-cache-phase1.php.<br /><br />The file %1$s/advanced-cache.php has been recreated and WPCACHEHOME fixed in your wp-config.php. Reload to hide this message.', 'wp-madx-cache' ), WP_CONTENT_DIR ) . '</p></div>';
		}
	}
}
...
function wp_cache_check_site() {
	global $wp_madx_cache_front_page_check, $wp_madx_cache_front_page_clear, $wp_madx_cache_front_page_text, $wp_madx_cache_front_page_notification, $wpdb;

	if ( !isset( $wp_madx_cache_front_page_check ) || ( isset( $wp_madx_cache_front_page_check ) && $wp_madx_cache_front_page_check == 0 ) ) {
		return false;
	}
	...
	if( is_array( $front_page ) ) {
		...
		if ( $front_page[ 'headers' ][ 'content-type' ] == 'application/x-gzip' ) {
			if ( !isset( $wp_madx_cache_front_page_clear ) || ( isset( $wp_madx_cache_front_page_clear ) && $wp_madx_cache_front_page_clear == 0 ) ) {
				wp_mail( get_option( 'admin_email' ), sprintf( __( '[%s] Front page is gzipped! Please clear cache!', 'wp-madx-cache' ), home_url() ), sprintf( __( "Please visit %s to clear the cache as the front page of your site is now downloading!", 'wp-madx-cache' ), admin_url( 'options-general.php?page=wpmadxcache' ) ) );
			} else {
				wp_cache_clear_cache( $wpdb->blogid );
				wp_mail( get_option( 'admin_email' ), sprintf( __( '[%s] Front page is gzipped! Cache Cleared!', 'wp-madx-cache' ), home_url() ), sprintf( __( "The cache on your blog has been cleared because the front page of your site is now downloading. Please visit %s to verify the cache has been cleared.", 'wp-madx-cache' ), admin_url( 'options-general.php?page=wpmadxcache' ) ) );
			}
		}

		// Check for broken front page
		if ( isset( $wp_madx_cache_front_page_text ) && $wp_madx_cache_front_page_text != '' && false === strpos( $front_page[ 'body' ], $wp_madx_cache_front_page_text ) ) {
			if ( !isset( $wp_madx_cache_front_page_clear ) || ( isset( $wp_madx_cache_front_page_clear ) && $wp_madx_cache_front_page_clear == 0 ) ) {
				wp_mail( get_option( 'admin_email' ), sprintf( __( '[%s] Front page is not correct! Please clear cache!', 'wp-madx-cache' ), home_url() ), sprintf( __( 'Please visit %1$s to clear the cache as the front page of your site is not correct and missing the text, "%2$s"!', 'wp-madx-cache' ), admin_url( 'options-general.php?page=wpmadxcache' ), $wp_madx_cache_front_page_text ) );
			} else {
				wp_cache_clear_cache( $wpdb->blogid );
				wp_mail( get_option( 'admin_email' ), sprintf( __( '[%s] Front page is not correct! Cache Cleared!', 'wp-madx-cache' ), home_url() ), sprintf( __( 'The cache on your blog has been cleared because the front page of your site is missing the text "%2$s". Please visit %1$s to verify the cache has been cleared.', 'wp-madx-cache' ), admin_url( 'options-general.php?page=wpmadxcache' ), $wp_madx_cache_front_page_text ) );
			}
		}
	}
	if ( isset( $wp_madx_cache_front_page_notification ) && $wp_madx_cache_front_page_notification == 1 ) {
		wp_mail( get_option( 'admin_email' ), sprintf( __( '[%s] Front page check!', 'wp-madx-cache' ), home_url() ), sprintf( __( "WP madx Cache has checked the front page of your blog. Please visit %s if you would like to disable this.", 'wp-madx-cache' ) . "\n\n", admin_url( 'options-general.php?page=wpmadxcache' ) ) );
	}
...
function wpsc_update_htaccess() {
	...
	wpsc_remove_marker( $home_path.'.htaccess', 'WordPress' ); // remove original WP rules so madxCache rules go on top
	if( insert_with_markers( $home_path.'.htaccess', 'WPmadxCache', explode( "\n", $rules ) ) && insert_with_markers( $home_path.'.htaccess', 'WordPress', explode( "\n", $wprules ) ) ) {
		return true;
	} else {
		return false;
	}
...
function wpsc_update_htaccess_form( $short_form = true ) {
	...
	if( !is_writeable_ACLSafe( $home_path . ".htaccess" ) ) {
		echo "<div style='padding:0 8px;color:#9f6000;background-color:#feefb3;border:1px solid #9f6000;'><h5>" . __( 'Cannot update .htaccess', 'wp-madx-cache' ) . "</h5><p>" . sprintf( __( 'The file <code>%s.htaccess</code> cannot be modified by the web server. Please correct this using the chmod command or your ftp client.', 'wp-madx-cache' ), $home_path ) . "</p><p>" . __( 'Refresh this page when the file permissions have been modified.' ) . "</p><p>" . sprintf( __( 'Alternatively, you can edit your <code>%s.htaccess</code> file manually and add the following code (before any WordPress rules):', 'wp-madx-cache' ), $home_path ) . "</p>";
		echo "<p><pre># BEGIN WPmadxCache\n" . esc_html( $rules ) . "# END WPmadxCache</pre></p></div>";
	} else {
		if ( $short_form == false ) {
			echo "<p>" . sprintf( __( 'To serve static html files your server must have the correct mod_rewrite rules added to a file called <code>%s.htaccess</code>', 'wp-madx-cache' ), $home_path ) . " ";
			_e( "You can edit the file yourself. Add the following rules.", 'wp-madx-cache' );
			echo __( " Make sure they appear before any existing WordPress rules. ", 'wp-madx-cache' ) . "</p>";
			...
			echo "<pre># BEGIN WPmadxCache\n" . esc_html( $rules ) . "# END WPmadxCache</pre></p>";
			echo "</div>";
			echo "<h5>" . sprintf( __( 'Rules must be added to %s too:', 'wp-madx-cache' ), WP_CONTENT_DIR . "/cache/.htaccess" ) . "</h5>";
			echo "<div style='overflow: auto; width: 800px; height: 400px; padding:0 8px;color:#9f6000;background-color:#feefb3;border:1px solid #9f6000;'>";
			echo "<pre># BEGIN madxcache\n" . esc_html( $gziprules ) . "# END madxcache</pre></p>";
			echo "</div>";
		}
		if ( !isset( $wpmu_version ) || $wpmu_version == '' ) {
			...
			echo '<div class="submit"><input class="button-primary" type="submit" ' . SUBMITDISABLED . 'id="updatehtaccess" value="' . __( 'Update Mod_Rewrite Rules', 'wp-madx-cache' ) . '" /></div>';
			wp_nonce_field('wp-cache');
			echo "</form>\n";
		}
	}
...
function wpsc_get_htaccess_info() {
	...
	if ( strpos( $document_root, '/kunden/homepages/' ) === 0 ) {
		// https://wordpress.org/support/topic/plugin-wp-madx-cache-how-to-get-mod_rewrite-working-on-1and1-shared-hosting?replies=1
	...
	$condition_rules = apply_filters( 'madxcacherewriteconditions', $condition_rules );
	...
	$rules .= "RewriteCond {$apache_root}{$inst_root}cache/madxcache/%{SERVER_NAME}{$home_root_lc}$1/index-https.html.gz -f\n";
	$rules .= "RewriteRule ^(.*) \"{$inst_root}cache/madxcache/%{SERVER_NAME}{$home_root_lc}$1/index-https.html.gz\" [L]\n\n";

	$rules .= "CONDITION_RULES";
	$rules .= "RewriteCond %{HTTP:Accept-Encoding} gzip\n";
	$rules .= "RewriteCond %{HTTPS} !on\n";
	$rules .= "RewriteCond {$apache_root}{$inst_root}cache/madxcache/%{SERVER_NAME}{$home_root_lc}$1/index.html.gz -f\n";
	$rules .= "RewriteRule ^(.*) \"{$inst_root}cache/madxcache/%{SERVER_NAME}{$home_root_lc}$1/index.html.gz\" [L]\n\n";

	$rules .= "CONDITION_RULES";
	$rules .= "RewriteCond %{HTTPS} on\n";
	$rules .= "RewriteCond {$apache_root}{$inst_root}cache/madxcache/%{SERVER_NAME}{$home_root_lc}$1/index-https.html -f\n";
	$rules .= "RewriteRule ^(.*) \"{$inst_root}cache/madxcache/%{SERVER_NAME}{$home_root_lc}$1/index-https.html\" [L]\n\n";

	$rules .= "CONDITION_RULES";
	$rules .= "RewriteCond %{HTTPS} !on\n";
	$rules .= "RewriteCond {$apache_root}{$inst_root}cache/madxcache/%{SERVER_NAME}{$home_root_lc}$1/index.html -f\n";
	$rules .= "RewriteRule ^(.*) \"{$inst_root}cache/madxcache/%{SERVER_NAME}{$home_root_lc}$1/index.html\" [L]\n";
	$rules .= "</IfModule>\n";
	$rules = apply_filters( 'madxcacherewriterules', $rules );
...
function clear_post_madxcache( $post_id ) {
	$dir = get_current_url_madxcache_dir( $post_id );
	if ( false == @is_dir( $dir ) )
		return false;

	if ( get_madxcache_dir() == $dir ) {
		wp_cache_debug( "clear_post_madxcache: not deleting post_id $post_id as it points at homepage: $dir" );
		return false;
	}

	wp_cache_debug( "clear_post_madxcache: post_id: $post_id. deleting files in $dir" );
	if ( get_post_type( $post_id ) != 'page') { // don't delete child pages if they exist
		prune_madx_cache( $dir, true );
	} else {
...
function wp_cron_preload_cache() {
	...
	if ( $wp_cache_preload_email_me && $c == 0 )
		wp_mail( get_option( 'admin_email' ), sprintf( __( '[%1$s] Cache Preload Started', 'wp-madx-cache' ), home_url(), '' ), ' ' );
			...
			foreach( $taxonomies as $taxonomy => $path ) {
				...
				if ( count( $details ) != 1 && $details[ 0 ] != '' ) {
					...
					if ( $wp_cache_preload_email_me && $wp_cache_preload_email_volume == 'many' )
						wp_mail( get_option( 'admin_email' ), sprintf( __( '[%1$s] Refreshing %2$s taxonomy from %3$d to %4$d', 'wp-madx-cache' ), home_url(), $taxonomy, $c, ($c+100) ), 'Refreshing: ' . print_r( $rows, 1 ) );
					foreach( (array)$rows as $url ) {
						set_time_limit( 60 );
						if ( $url == '' )
							continue;
						$url_info = parse_url( $url );
						$dir = get_madxcache_dir() . $url_info[ 'path' ];
						...
						if ( @file_exists( $cache_path . "stop_preload.txt" ) ) {
							...
							if ( $wp_cache_preload_email_me )
								wp_mail( get_option( 'admin_email' ), sprintf( __( '[%1$s] Cache Preload Stopped', 'wp-madx-cache' ), home_url(), '' ), ' ' );
							return true;
						}
					}
					$fp = fopen( $taxonomy_filename, 'w' );
	...
	if ( $posts ) {
		...
		if ( $wp_cache_preload_email_me && $wp_cache_preload_email_volume == 'many' )
			wp_mail( get_option( 'admin_email' ), sprintf( __( '[%1$s] Refreshing posts from %2$d to %3$d', 'wp-madx-cache' ), home_url(), $c, ($c+100) ), ' ' );
		$msg = '';
		$count = $c + 1;
		$permalink_counter_msg = $cache_path . "preload_permalink.txt";
		...
		if ( $wp_cache_preload_email_me && ( $wp_cache_preload_email_volume == 'medium' || $wp_cache_preload_email_volume == 'many' ) )
			wp_mail( get_option( 'admin_email' ), sprintf( __( '[%1$s] %2$d posts refreshed', 'wp-madx-cache' ), home_url(), ($c+100) ), __( "Refreshed the following posts:", 'wp-madx-cache' ) . "\n$msg" );
		...
		wpsc_delete_files( get_madxcache_dir() );
	} else {
		$msg = '';
		update_option( 'preload_cache_counter', array( 'c' => 0, 't' => time() ) );
		if ( (int)$wp_cache_preload_interval && defined( 'DOING_CRON' ) ) {
			if ( $wp_cache_preload_email_me )
				$msg = sprintf( __( 'Scheduling next preload refresh in %d minutes.', 'wp-madx-cache' ), (int)$wp_cache_preload_interval );
		...
		if ( $wp_cache_preload_email_me )
			wp_mail( get_option( 'admin_email' ), sprintf( __( '[%s] Cache Preload Completed', 'wp-madx-cache' ), home_url() ), __( "Cleaning up old madxcache files.", 'wp-madx-cache' ) . "\n" . $msg );
	...
	if ( is_array( $value ) && $value[ 'c' ] > 0 && ( time() - $value[ 't' ] ) > 3600 && false == wp_next_scheduled( 'wp_cache_preload_hook' ) ) {
		if ( is_admin() ) {
			if ( get_option( 'wpsc_preload_restart_email' ) < ( time() - 86400 ) ) {
				wp_mail( get_option( 'admin_email' ), sprintf( __( '[%s] Preload may have stalled.', 'wp-madx-cache' ), get_bloginfo( 'url' ) ), sprintf( __( "Preload has been restarted.\n%s", 'wp-madx-cache' ), admin_url( "options-general.php?page=wpmadxcache" ) ) );
...
function wpsc_preload_restart_notice() {
	...
	if ( ! isset( $_GET[ 'page' ] ) || $_GET[ 'page' ] != 'wpmadxcache' )
		return false;
	echo '<div class="notice notice-error"><p>' . __( 'Warning! WP madx Cache preload was interrupted but has been restarted.', 'wp-madx-cache' ) . '</p></div>';
}
...
function wp_cache_disable_plugin( $delete_config_file = true ) {
	...
	uninstall_madxcache( WP_CONTENT_DIR . '/cache' );
	...
	if ( $file_not_deleted ) {
		$msg = __( "Dear User,\n\nWP madx Cache was removed from your blog or deactivated but some files could\nnot be deleted.\n\n", 'wp-madx-cache' );
		foreach( (array)$file_not_deleted as $path ) {
			$msg .=  "{$path}\n";
		}
		$msg .= "\n";
		$msg .= sprintf( __( "You should delete these files manually.\nYou may need to change the permissions of the files or parent directory.\nYou can read more about this in the Codex at\n%s\n\nThank you.", 'wp-madx-cache' ), 'https://codex.wordpress.org/Changing_File_Permissions#About_Chmod' );

		if ( apply_filters( 'wpsc_send_uninstall_errors', 1 ) ) {
			wp_mail( get_option( 'admin_email' ), __( 'WP madx Cache: could not delete files', 'wp-madx-cache' ), $msg );
		}
	}
	extract( wpsc_get_htaccess_info() ); // $document_root, $apache_root, $home_path, $home_root, $home_root_lc, $inst_root, $wprules, $scrules, $condition_rules, $rules, $gziprules
	if ( $scrules != '' && insert_with_markers( $home_path.'.htaccess', 'WPmadxCache', array() ) ) {
		$wp_rewrite->flush_rules();
	} elseif( $scrules != '' ) {
		wp_mail( get_option( 'admin_email' ), __( 'madxcache Uninstall Problems', 'wp-madx-cache' ), sprintf( __( "Dear User,\n\nWP madx Cache was removed from your blog but the mod_rewrite rules\nin your .htaccess were not.\n\nPlease edit the following file and remove the code\nbetween 'BEGIN WPmadxCache' and 'END WPmadxCache'. Please backup the file first!\n\n%s\n\nRegards,\nWP madx Cache Plugin\nhttps://wordpress.org/plugins/wp-madx-cache/", 'wp-madx-cache' ), ABSPATH . '/.htaccess' ) );
	}
}

function uninstall_madxcache( $folderPath ) { // from http://www.php.net/manual/en/function.rmdir.php
	if ( trailingslashit( constant( 'ABSPATH' ) ) == trailingslashit( $folderPath ) )
		return false;
	if ( @is_dir ( $folderPath ) ) {
		$dh  = @opendir($folderPath);
...
function madxcache_admin_bar_render() {
	global $wp_admin_bar;
...
function wpsc_preload_settings( $min_refresh_interval = 'NA' ) {
	...
	if ( isset( $_POST[ 'preload_off' ] ) ) {
		wpsc_cancel_preload();
		$return[] = "<p><strong>" . __( 'Scheduled preloading of cache almost cancelled. It may take up to a minute for it to cancel completely.', 'wp-madx-cache' ) . "</strong></p>";
		return $return;
...
function update_mod_rewrite_rules( $add_rules = true ) {
	...
	if ( empty( $rules ) ) {
		return insert_with_markers( $home_path . '.htaccess', 'WPmadxCache', array() );
	}
	...
	$existing_gzip_rules = implode( "\n", extract_from_markers( $cache_path . '.htaccess', 'madxcache' ) );
	if ( $existing_gzip_rules != $generated_rules[ 'gziprules' ] ) {
		insert_with_markers( $cache_path . '.htaccess', 'madxcache', explode( "\n", $generated_rules[ 'gziprules' ] ) );
	}
...
function wpsc_feed_update( $type, $permalink ) {
	...
	if ( is_array( $wpsc_feed_list ) && ! empty( $wpsc_feed_list ) ) {
		foreach( $wpsc_feed_list as $file ) {
			...
			prune_madx_cache( $file, true );
			prune_madx_cache( dirname( $file ) . '/meta-' . basename( $file ), true );
		}
	}
}
