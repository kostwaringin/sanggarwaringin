jQuery( document ).ready( () => {
	...
	jQuery( '#wpsc-install-boost-button' ).on( 'click', event => {
		...
		showBoostBannerBusy( __( 'Installing…', 'wp-madx-cache' ) );
			...
			.fail( response => {
				showBoostBannerError(
					sprintf(
						...
						__( 'Failed to install Jetpack Boost: HTTP %d error received', 'wp-madx-cache' ),
						response.status
					)
				);
			} );
	...
	const resetBoostBannerButton = () => {
		...
		jQuery( '#wpsc-activate-boost-button' )
			.find( 'label' )
			.text( __( 'Activate Jetpack Boost', 'wp-madx-cache' ) );
		jQuery( '#wpsc-install-boost-button' )
			.find( 'label' )
			.text( __( 'Install Jetpack Boost', 'wp-madx-cache' ) );
	...
	const showBoostBannerError = err => {
		...
		jQuery( '#wpsc-boost-banner-error' )
			.text(
				err || __( 'An error occurred while trying to activate Jetpack Boost', 'wp-madx-cache' )
			)
			.show();
	};
	...
	const activateBoost = () => {
		showBoostBannerBusy( __( 'Activating…', 'wp-madx-cache' ) );
		...
		jQuery
			...
			.fail( response => {
				showBoostBannerError(
					sprintf(
						/* translators: %d is an HTTP error code */
						__( 'Failed to activate Jetpack Boost: HTTP %d error received', 'wp-madx-cache' ),
						response.status
					)
				);
			} );
