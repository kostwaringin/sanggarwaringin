...
require_once __DIR__ . '/class.wp-madx-cache-settings-map.php';

class WP_madx_Cache_Rest_Update_Settings extends WP_REST_Controller {
	...
	public function callback( $request ) {
		...
		if ( ! empty( $errors ) ) {
			...
			$get_settings = new WP_madx_Cache_Rest_Get_Settings();
	...
	protected function set_value_by_key( $value, $key ) {
		...
		$settings_map = WP_madx_Cache_Settings_Map::$map;
	 ...
	 * @param mixed $value
	 */
	protected function set_madx_cache_enabled( $value ) {
	...
	protected function set_wp_cache_not_logged_in( $value ) {
		...
		if ( 0 != $value ) {
			if ( 0 == $wp_cache_not_logged_in && function_exists( 'prune_madx_cache' ) ) {
	...
	protected function set_cache_compression( $value ) {
		...
		if ( defined( 'WPSC_DISABLE_COMPRESSION' ) ) {
			...
			if ( 1 == ini_get( 'zlib.output_compression' ) || "on" == strtolower( ini_get( 'zlib.output_compression' ) ) ) {
				return __( "You attempted to enable compression but `zlib.output_compression` is enabled. See #21 in the Troubleshooting section of the readme file.", 'wp-madx-cache' );
			}
	 ...
	 * @param mixed $value
	 */
	protected function set_madx_cname( $value ) {
		update_option( 'madx_cname', $value );
	}

	/**
	 * @param mixed $value
	 */
	protected function set_madx_off_blog_url( $value ) {
		update_option( 'madx_off_blog_url', untrailingslashit( $value ) );
	}

	/**
	 * @param mixed $value
	 */
	protected function set_madx_off_cdn_url( $value ) {
		update_option( 'madx_off_cdn_url', $value );
	}

	/**
	 * @param mixed $value
	 */
	protected function set_madx_off_include_dirs( $value ) {
		update_option( 'madx_off_include_dirs', $value == '' ? 'wp-content,wp-includes' : $value );
	}

	/**
	 * @param mixed $value
	 */
	protected function set_madx_off_exclude( $value ) {
		update_option( 'madx_off_exclude', $value );
	}

	/**
	 * @param mixed $value
	 */
	protected function set_madx_https( $value ) {
		update_option( 'madx_https', $value ? 1 : 0 );
	}

	/**
	 * @param mixed $value
	 */
	protected function set_madxcdn( $value ) {
		global $wp_cache_config_file;

		$madxcdn = $value ? 1 : 0;
		wp_cache_replace_line( '^ *\$madxcdn', "\$madxcdn = $madxcdn;", $wp_cache_config_file );
	}
	...
	public function set_debug_settings( $parameters ) {
		...
		$settings = array (
			'wp_madx_cache_debug',
			'wp_cache_debug_ip',
			'wp_madx_cache_comments',
			'wp_madx_cache_front_page_check',
			'wp_madx_cache_front_page_clear',
			'wp_madx_cache_front_page_text',
			'wp_madx_cache_front_page_notification',
			'wpsc_delete_log',
		);
