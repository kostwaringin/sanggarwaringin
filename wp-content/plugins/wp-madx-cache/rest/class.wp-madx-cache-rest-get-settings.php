...
class WP_madx_Cache_Rest_Get_Settings extends WP_REST_Controller {
	...
	public function callback( $request ) {
		...
		foreach ( WP_madx_Cache_Settings_Map::$map as $name => $map ) {
			if ( isset ( $map['get'] ) ) {
				$get_method = $map['get'];
	 ...
	 * @return string
	 */
	public function get_madx_off_blog_url() {
		$url = get_option( 'madx_off_blog_url' );
		if ( ! $url )
			$url = apply_filters( 'madx_off_blog_url', untrailingslashit( get_option( 'siteurl' ) ) );
		return $url;
	}
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$strings_to_bool = array( 'madx_https', 'refresh_current_only_on_comments' );
	...
	protected function get_is_preload_enabled() {
		return false === defined( 'DISABLEmadxCACHEPRELOADING' );
	}
