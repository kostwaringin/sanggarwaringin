...
class WP_madx_Cache_Rest_Get_Status extends WP_REST_Controller {
	...
	protected function add_preload_status( & $status ) {
		...
		if ( false == $madx_cache_enabled ) {
			$status[ 'preload_disabled_madxcache_off' ] = true;
		}
		if ( true === defined( 'DISABLEmadxCACHEPRELOADING' ) ) {
	...
	protected function add_rewrite_status( & $status ) {
		...
		if ( ! $cache_enabled || ! $madx_cache_enabled || ! $wp_cache_mod_rewrite ) {
			return;
		}
