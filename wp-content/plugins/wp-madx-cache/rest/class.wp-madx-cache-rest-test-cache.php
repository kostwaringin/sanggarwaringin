...
class WP_madx_Cache_Rest_Test_Cache extends WP_REST_Controller {
	...
	public function callback( $request ) {
		...
		foreach ( $attempts as $attempt_name ) {
			...
			} elseif ( 0 !== $c && 0 === preg_match( '/(Cached page generated by WP-madx-Cache on) ([0-9]*-[0-9]*-[0-9]* [0-9]*:[0-9]*:[0-9]*)/', $page[ $c ]['body'], $matches2 ) ) {
				$has_errors = true;
				$attempt['status'] = false;
				$attempt['errors'] = array( __( 'Timestamps not found', 'wp-madx-cache' ) );
		...
		if (
			false == $has_errors &&
			preg_match( '/(Cached page generated by WP-madx-Cache on) ([0-9]*-[0-9]*-[0-9]* [0-9]*:[0-9]*:[0-9]*)/', $page[ 1 ][ 'body' ], $matches1 ) &&
			preg_match( '/(Cached page generated by WP-madx-Cache on) ([0-9]*-[0-9]*-[0-9]* [0-9]*:[0-9]*:[0-9]*)/', $page[ 2 ][ 'body' ], $matches2 ) &&
			$matches1[2] == $matches2[2]
		) {
			$response[ 'status' ] = true;
		} else {
			$response[ 'status' ] = false;
			$response[ 'error' ] = array( __( 'Timestamps do not match', 'wp-madx-cache' ) );
		}
