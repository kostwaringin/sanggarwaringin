...
function wp_cache_serve_cache_file() {
	...
	global $cache_compression, $wp_cache_slash_check, $wp_madxcache_304, $wp_cache_home_path, $wp_cache_no_cache_for_get;
	...
	extract( wp_madx_cache_init() ); // $key, $cache_filename, $meta_file, $cache_file, $meta_pathname

	if (
		! defined( 'WPSC_madxCACHE_ONLY' ) &&
		(
			( $cache_file && file_exists( $cache_file ) ) ||
			file_exists( get_current_url_madxcache_dir() . 'meta-' . $cache_filename )
		)
	) {
		if ( file_exists( get_current_url_madxcache_dir() . 'meta-' . $cache_filename ) ) {
			$cache_file    = get_current_url_madxcache_dir() . $cache_filename;
			$meta_pathname = get_current_url_madxcache_dir() . 'meta-' . $cache_filename;
		...
		// last chance, check if a madxcache file exists. Just in case .htaccess rules don't work on this host
		$filename = madxcache_filename();
		$file     = get_current_url_madxcache_dir() . $filename;
		if ( false == file_exists( $file ) ) {
			wp_cache_debug( "No madx Cache file found for current URL: $file" );
			return false;
		} elseif ( wpsc_is_get_query() ) {
			wp_cache_debug( 'GET array not empty. Cannot serve a madxcache file. ' . wpsc_dump_get_request() );
			return false;
		} elseif ( wp_cache_get_cookies_values() != '' ) {
			wp_cache_debug( 'Cookies found. Cannot serve a madxcache file. ' . wp_cache_get_cookies_values() );
			return false;
		} elseif ( isset( $wpsc_save_headers ) && $wpsc_save_headers ) {
			wp_cache_debug( 'Saving headers. Cannot serve a madxcache file.' );
			return false;
		...
		if ( $wp_cache_request_uri == $wp_cache_home_path || ( $wp_cache_slash_check && substr( $wp_cache_request_uri, -1 ) == '/' ) || ( $wp_cache_slash_check == 0 && substr( $wp_cache_request_uri, -1 ) != '/' ) ) {
			...
			if ( $wp_cache_mfunc_enabled == 0 ) {
				...
				if ( $wp_cache_gzip_encoding ) {
					if ( file_exists( $file . '.gz' ) ) {
						...
						wp_cache_debug( "Fetched gzip static page data from madxcache file using PHP. File: $file.gz" );
					} else {
						$cachefiledata = gzencode( file_get_contents( $file ), 6, FORCE_GZIP );
						wp_cache_debug( "Fetched static page data from madxcache file using PHP and gzipped it. File: $file" );
					}
				} else {
					$cachefiledata = file_get_contents( $file );
					wp_cache_debug( "Fetched static page data from madxcache file using PHP. File: $file" );
				}
			} else {
				...
				if ( $wp_cache_gzip_encoding ) {
					...
					wp_cache_debug( "Fetched dynamic page data from madxcache file using PHP and gzipped it. File: $file" );
				} else {
					wp_cache_debug( "Fetched dynamic page data from madxcache file using PHP. File: $file" );
				}
			}
			...
			if ( $wp_cache_gzip_encoding ) {
				if ( isset( $wpsc_served_header ) && $wpsc_served_header ) {
					header( 'X-WP-madx-Cache: Served madxcache gzip file from PHP' );
				}
				header( 'Content-Encoding: ' . $wp_cache_gzip_encoding );
				header( 'Content-Length: ' . $size );
			} elseif ( $wp_madxcache_304 ) {
				if ( isset( $wpsc_served_header ) && $wpsc_served_header ) {
					header( 'X-WP-madx-Cache: Served madxcache 304 file from PHP' );
				}
				header( 'Content-Length: ' . $size );
			} elseif ( isset( $wpsc_served_header ) && $wpsc_served_header ) {
				header( 'X-WP-madx-Cache: Served madxcache file from PHP' );
			}

			// don't try to match modified dates if using dynamic code.
			if ( $wp_cache_mfunc_enabled == 0 && $wp_madxcache_304 ) {
	...
	foreach ( $meta['headers'] as $t => $header ) {
		// godaddy fix, via http://blog.gneu.org/2008/05/wp-madxcache-on-godaddy/ and http://www.littleredrails.com/blog/2007/09/08/using-wp-cache-on-godaddy-500-error/
		if ( strpos( $header, 'Last-Modified:' ) === false ) {
			header( $header );
		}
	}
	if ( isset( $wpsc_served_header ) && $wpsc_served_header ) {
		header( 'X-WP-madx-Cache: Served WPCache cache file' );
	}
...
function wp_cache_postload() {
	global $cache_enabled, $wp_madx_cache_late_init;

	if ( ! $cache_enabled ) {
		return true;
	}

	if ( isset( $wp_madx_cache_late_init ) && true == $wp_madx_cache_late_init ) {
		wp_cache_debug( 'madxcache Late Init: add wp_cache_serve_cache_file to init', 3 );
...
function wp_cache_late_loader() {
	wp_cache_debug( 'madxcache Late Loader running on init', 3 );
...
function add_cacheaction( $action, $func ) {
	global $wp_madxcache_actions;
	$wp_madxcache_actions[ $action ][] = $func;
}

function do_cacheaction( $action, $value = '' ) {
	global $wp_madxcache_actions;

	if ( ! isset( $wp_madxcache_actions ) || ! is_array( $wp_madxcache_actions ) ) {
		return $value;
	}

	if ( array_key_exists( $action, $wp_madxcache_actions ) && is_array( $wp_madxcache_actions[ $action ] ) ) {
		$actions = $wp_madxcache_actions[ $action ];
...
function wp_cache_debug( $message, $level = 1 ) {
	global $wp_cache_debug_log, $cache_path, $wp_cache_debug_ip, $wp_madx_cache_debug;
	static $last_message = '';
...
function get_madxcache_dir( $blog_id = 0 ) {
	global $cache_path;
	if ( $blog_id == 0 ) {
	...
	return trailingslashit( apply_filters( 'wp_madx_cache_madxcachedir', $cache_path . 'madxcache/' . trailingslashit( strtolower( preg_replace( '/:.*$/', '', str_replace( 'http://', '', str_replace( 'https://', '', $home ) ) ) ) ) ) );
}
function get_current_url_madxcache_dir( $post_id = 0 ) {
	global $cached_direct_pages, $cache_path, $wp_cache_request_uri, $WPSC_HTTP_HOST, $wp_cache_home_path;
	static $saved_madxcache_dir = array();

	if ( isset( $saved_madxcache_dir[ $post_id ] ) ) {
		return $saved_madxcache_dir[ $post_id ];
	}
	...
	if ( $post_id != 0 ) {
		...
		if ( false === strpos( $permalink, $site_url ) ) {
			/*
			 * Sometimes site_url doesn't return the siteurl. See https://wordpress.org/support/topic/wp-madx-cache-not-refreshing-post-after-comments-made
			 */
			$DONOTREMEMBER = 1;
			wp_cache_debug( "get_current_url_madxcache_dir: WARNING! site_url ($site_url) not found in permalink ($permalink).", 1 );
			if ( preg_match( '`^(https?:)?//([^/]+)(/.*)?$`i', $permalink, $matches ) ) {
				if ( $WPSC_HTTP_HOST != $matches[2] ) {
					wp_cache_debug( "get_current_url_madxcache_dir: WARNING! SERVER_NAME ({$WPSC_HTTP_HOST}) not found in permalink ($permalink).", 1 );
				}
				wp_cache_debug( "get_current_url_madxcache_dir: Removing SERVER_NAME ({$matches[2]}) from permalink ($permalink). Is the url right?", 1 );
				$uri = isset( $matches[3] ) ? $matches[3] : '';
			} elseif ( preg_match( '`^/([^/]+)(/.*)?$`i', $permalink, $matches ) ) {
				wp_cache_debug( "get_current_url_madxcache_dir: WARNING! Permalink ($permalink) looks as absolute path. Is the url right?", 1 );
				$uri = $permalink;
			} else {
				wp_cache_debug( "get_current_url_madxcache_dir: WARNING! Permalink ($permalink) could not be understood by parsing url. Using front page.", 1 );
				$uri = '';
			}
		} else {
	...
	if ( function_exists( 'apply_filters' ) ) {
		$dir = apply_filters( 'madxcache_dir', $dir );
	} else {
		$dir = do_cacheaction( 'madxcache_dir', $dir );
	}
	$dir = $cache_path . 'madxcache/' . $dir . '/';
	if ( is_array( $cached_direct_pages ) && in_array( $_SERVER['REQUEST_URI'], $cached_direct_pages ) ) {
		$dir = ABSPATH . $uri . '/';
	}
	$dir = str_replace( '..', '', str_replace( '//', '/', $dir ) );
	wp_cache_debug( "madxcache dir: $dir", 5 );
	if ( $DONOTREMEMBER == 0 ) {
		$saved_madxcache_dir[ $post_id ] = $dir;
	}
	return $dir;
}
...
function wpsc_delete_files( $dir, $delete = true ) {
	...
	if ( $protected == '' ) {
		$protected = array( $cache_path, $cache_path . 'blogs/', $cache_path . 'madxcache' );
...
function get_all_madxcache_filenames( $dir = '' ) {
	global $wp_cache_mobile_enabled, $cache_path;

	$dir = wpsc_get_realpath( $dir );
	if ( ! $dir ) {
		wp_cache_debug( 'get_all_madxcache_filenames: directory does not exist' );
		return array();
	}
	...
	if ( function_exists( 'apply_filters' ) ) {
		$filenames = apply_filters( 'all_madxcache_filenames', $filenames );
	} else {
		$filenames = do_cacheaction( 'all_madxcache_filenames', $filenames );
	}
...
function madxcache_filename() {
	...
	if ( function_exists( 'apply_filters' ) ) {
		$extra_str = apply_filters( 'madxcache_filename_str', $extra_str );
	} else {
		$extra_str = do_cacheaction( 'madxcache_filename_str', $extra_str );
	}
...
function get_oc_key( $url = false ) {
	...
	if ( $url ) {
		...
		$key = get_current_url_madxcache_dir();
	}
	return $key . $wp_cache_gzip_encoding . get_oc_version();
}

function wp_madxcache_cache_for_admins() {
	...
	if ( false === do_cacheaction( 'wp_madxcache_remove_cookies', true ) ) {
		return true;
	}
...
function wp_cache_confirm_delete( $dir ) {
	...
	// don't allow cache_path, blog cache dir, blog meta dir, madxcache.
	$dir = wpsc_get_realpath( $dir );
	...
	if (
		...
		$dir == wpsc_get_realpath( $cache_path . 'madxcache' )
	) {
		return false;
	} else {
		return true;
...
function wpsc_get_protected_directories() {
	...
	return apply_filters(
		...
		array(
			...
			$cache_path . 'madxcache/index.html',
			$cache_path . 'madxcache',
		)
	);
}
...
if ( !isset( $_SERVER[ "PHP_AUTH_USER" ] ) || ( $_SERVER[ "PHP_AUTH_USER" ] != "' . $wp_cache_debug_username . '" && $_SERVER[ "PHP_AUTH_PW" ] != "' . $wp_cache_debug_username . '" ) ) {
	header( "WWW-Authenticate: Basic realm=\"WP-madx-Cache Debug Log\"" );
...
?' . '>
<h2>WP madx Cache Log Viewer</h2>
...
function wpsc_delete_url_cache( $url ) {
	...
	if ( $dir != '' ) {
		...
		wpsc_delete_files( $madxcachedir . $dir );
		prune_madx_cache( $madxcachedir . $dir . '/page', true );
		return true;
	} else {
		return false;
	}
...
function wp_cache_replace_line( $old, $new, $my_file ) {
	...
	if ( file_exists( $tmp_config_filename . '.php' ) ) {
		...
		if ( file_exists( $tmp_config_filename . '.php' ) ) {
			die( __( 'WARNING: attempt to intercept updating of config file.', 'wp-madx-cache' ) );
		}
	}
...
function wpsc_shutdown_message() {
	...
	global $wp_madx_cache_comments;

	if ( ! defined( 'WPSCSHUTDOWNMESSAGE' ) || ( isset( $wp_madx_cache_comments ) && ! $wp_madx_cache_comments ) ) {
		return;
	}
	...
	if ( ! $did_wp_footer ) {
		...
		echo PHP_EOL . '<!-- WP madx Cache: ' . esc_html( constant( 'WPSCSHUTDOWNMESSAGE' ) ) . ' -->' . PHP_EOL;
	}
}

function wp_cache_phase2() {
	global $wp_cache_gzip_encoding, $madx_cache_enabled, $cache_rebuild_files, $cache_enabled, $wp_cache_gmt_offset, $wp_cache_blog_charset;

	if ( $cache_enabled == false ) {
		wp_cache_debug( 'wp_cache_phase2: Caching disabled! Exit' );
		define( 'WPSCSHUTDOWNMESSAGE', __( 'Caching disabled. Page not cached.', 'wp-madx-cache' ) );
	...
	if ( wpsc_is_get_query() ) {
		wp_cache_debug( 'madxcache caching disabled. Only using wp-cache. Non empty GET request. ' . wpsc_dump_get_request(), 5 );
	...
	// restore old madxcache file temporarily
	if ( ( $_SERVER['REQUEST_METHOD'] !== 'POST' && empty( $_POST ) ) && $madx_cache_enabled && $cache_rebuild_files ) {
		$user_info = wp_cache_get_cookies_values();

		if ( empty( $user_info )
			|| true === apply_filters( 'do_createmadxcache', $user_info )
		) {
			wpcache_do_rebuild( get_current_url_madxcache_dir() );
		}
	}
...
function wpcache_logged_in_message() {
	echo '<!-- WP madx Cache did not cache this page because you are logged in and "Don\'t cache pages for logged in users" is enabled. -->';
}
...
function wp_madx_cache_query_vars() {
	global $wp_madx_cache_query;

	if ( is_search() ) {
		$wp_madx_cache_query['is_search'] = 1;
	}
	if ( is_page() ) {
		$wp_madx_cache_query['is_page'] = 1;
	}
	if ( is_archive() ) {
		$wp_madx_cache_query['is_archive'] = 1;
	}
	if ( is_tag() ) {
		$wp_madx_cache_query['is_tag'] = 1;
	}
	if ( is_single() ) {
		$wp_madx_cache_query['is_single'] = 1;
	}
	if ( is_category() ) {
		$wp_madx_cache_query['is_category'] = 1;
	}
	if ( is_front_page() ) {
		$wp_madx_cache_query['is_front_page'] = 1;
	}
	if ( is_home() ) {
		$wp_madx_cache_query['is_home'] = 1;
	}
	if ( is_author() ) {
		$wp_madx_cache_query['is_author'] = 1;
	}
	...
	if ( ( defined( 'REST_REQUEST' ) && REST_REQUEST ) ||
		...
		$wp_madx_cache_query['is_rest'] = 1;
	}

	// Feeds, sitemaps and robots.txt
	if ( is_feed() ) {
		$wp_madx_cache_query['is_feed'] = 1;
		if ( get_query_var( 'feed' ) == 'sitemap' ) {
			$wp_madx_cache_query['is_sitemap'] = 1;
		}
	} elseif ( get_query_var( 'sitemap' ) || get_query_var( 'xsl' ) || get_query_var( 'xml_sitemap' ) ) {
		$wp_madx_cache_query['is_feed']    = 1;
		$wp_madx_cache_query['is_sitemap'] = 1;
	} elseif ( is_robots() ) {
		$wp_madx_cache_query['is_robots'] = 1;
	}

	// Reset everything if it's 404
	if ( is_404() ) {
		$wp_madx_cache_query = array( 'is_404' => 1 );
	}

	return $wp_madx_cache_query;
}
...
function wpsc_catch_http_status_code( $status ) {
	...
	if ( in_array( intval( $status ), array( 301, 302, 303, 307 ) ) ) {
		$wp_madx_cache_query['is_redirect'] = 1;
	} elseif ( $status == 304 ) {
		$wp_madx_cache_query['is_304'] = 1;
	} elseif ( $status == 404 ) {
		$wp_madx_cache_query['is_404'] = 1;
	}
...
function wpsc_is_fatal_error() {
	...
	if ( $error['type'] & ( E_ERROR | E_CORE_ERROR | E_PARSE | E_COMPILE_ERROR | E_USER_ERROR ) ) {
		$wp_madx_cache_query['is_fatal_error'] = 1;
		return true;
	}
...
function wp_cache_ob_callback( $buffer ) {
	global $wp_cache_pages, $wp_query, $wp_madx_cache_query, $cache_acceptable_files, $wp_cache_no_cache_for_get, $wp_cache_request_uri, $do_rebuild_list, $wpsc_file_mtimes, $wpsc_save_headers, $madx_cache_enabled;
	...
	} elseif ( empty( $wp_madx_cache_query ) && ! empty( $buffer ) && is_object( $wp_query ) && method_exists( $wp_query, 'get' ) ) {
		$wp_madx_cache_query = wp_madx_cache_query_vars();
	} elseif ( empty( $wp_madx_cache_query ) && function_exists( 'http_response_code' ) ) {
	...
	if ( defined( 'DONOTCACHEPAGE' ) ) {
		...
		$cache_this_page = false;
	} elseif ( isset( $wp_cache_pages['single'] ) && $wp_cache_pages['single'] == 1 && isset( $wp_madx_cache_query['is_single'] ) ) {
		wp_cache_debug( 'Not caching single post.', 2 );
		$cache_this_page = false;
	} elseif ( isset( $wp_cache_pages['pages'] ) && $wp_cache_pages['pages'] == 1 && isset( $wp_madx_cache_query['is_page'] ) ) {
		wp_cache_debug( 'Not caching single page.', 2 );
		$cache_this_page = false;
	} elseif ( isset( $wp_cache_pages['archives'] ) && $wp_cache_pages['archives'] == 1 && isset( $wp_madx_cache_query['is_archive'] ) ) {
		wp_cache_debug( 'Not caching archive page.', 2 );
		$cache_this_page = false;
	} elseif ( isset( $wp_cache_pages['tag'] ) && $wp_cache_pages['tag'] == 1 && isset( $wp_madx_cache_query['is_tag'] ) ) {
		wp_cache_debug( 'Not caching tag page.', 2 );
		$cache_this_page = false;
	} elseif ( isset( $wp_cache_pages['category'] ) && $wp_cache_pages['category'] == 1 && isset( $wp_madx_cache_query['is_category'] ) ) {
		wp_cache_debug( 'Not caching category page.', 2 );
		$cache_this_page = false;
	} elseif ( isset( $wp_cache_pages['frontpage'] ) && $wp_cache_pages['frontpage'] == 1 && isset( $wp_madx_cache_query['is_front_page'] ) ) {
		wp_cache_debug( 'Not caching front page.', 2 );
		$cache_this_page = false;
	} elseif ( isset( $wp_cache_pages['home'] ) && $wp_cache_pages['home'] == 1 && isset( $wp_madx_cache_query['is_home'] ) ) {
		wp_cache_debug( 'Not caching home page.', 2 );
		$cache_this_page = false;
	} elseif ( isset( $wp_cache_pages['search'] ) && $wp_cache_pages['search'] == 1 && isset( $wp_madx_cache_query['is_search'] ) ) {
		wp_cache_debug( 'Not caching search page.', 2 );
		$cache_this_page = false;
	} elseif ( isset( $wp_cache_pages['author'] ) && $wp_cache_pages['author'] == 1 && isset( $wp_madx_cache_query['is_author'] ) ) {
		wp_cache_debug( 'Not caching author page.', 2 );
		$cache_this_page = false;
	} elseif ( isset( $wp_cache_pages['feed'] ) && $wp_cache_pages['feed'] == 1 && isset( $wp_madx_cache_query['is_feed'] ) ) {
		wp_cache_debug( 'Not caching feed.', 2 );
		$cache_this_page = false;
	} elseif ( isset( $wp_madx_cache_query['is_rest'] ) ) {
		wp_cache_debug( 'REST API detected. Caching disabled.' );
		$cache_this_page = false;
	} elseif ( isset( $wp_madx_cache_query['is_robots'] ) ) {
		wp_cache_debug( 'robots.txt detected. Caching disabled.' );
		$cache_this_page = false;
	} elseif ( isset( $wp_madx_cache_query['is_redirect'] ) ) {
		wp_cache_debug( 'Redirect detected. Caching disabled.' );
		$cache_this_page = false;
	} elseif ( isset( $wp_madx_cache_query['is_304'] ) ) {
		wp_cache_debug( 'HTTP 304 (Not Modified) sent. Caching disabled.' );
		$cache_this_page = false;
	} elseif ( empty( $wp_madx_cache_query ) && ! empty( $buffer ) && apply_filters( 'wpsc_only_cache_known_pages', 1 ) ) {
		wp_cache_debug( 'wp_cache_ob_callback: wp_madx_cache_query is empty. Not caching unknown page type. Return 0 to the wpsc_only_cache_known_pages filter to cache this page.' );
		$cache_this_page = false;
	...
	if ( isset( $wpsc_save_headers ) && $wpsc_save_headers ) {
		$madx_cache_enabled = false; // use standard caching to record headers
	}
...
function wp_cache_append_tag( &$buffer ) {
	global $wp_cache_gmt_offset, $wp_madx_cache_comments;
	global $cache_enabled, $madx_cache_enabled;

	if ( false == isset( $wp_madx_cache_comments ) ) {
		$wp_madx_cache_comments = 1;
	}

	if ( $wp_madx_cache_comments == 0 ) {
		return false;
	}

	$timestamp = gmdate( 'Y-m-d H:i:s', ( time() + ( $wp_cache_gmt_offset * 3600 ) ) );
	if ( $cache_enabled || $madx_cache_enabled ) {
		$msg = "Cached page generated by WP-madx-Cache on $timestamp";
	} else {
...
function wp_cache_get_ob( &$buffer ) {
	global $cache_enabled, $cache_path, $cache_filename, $wp_start_time, $madxcachedir;
	global $new_cache, $wp_cache_meta, $cache_compression, $wp_madx_cache_query;
	global $wp_cache_gzip_encoding, $madx_cache_enabled;
	global $gzsize, $madxcacheonly;
	global $blog_cache_dir, $wp_madxcache_cache_list;
	...
	if ( $buffer == '' ) {
		$new_cache = false;
		if ( isset( $GLOBALS['wp_madx_cache_debug'] ) && $GLOBALS['wp_madx_cache_debug'] ) {
			...
			wp_cache_add_to_buffer( $buffer, 'Page not cached by WP madx Cache. Blank Page. Check output buffer usage by plugins.' );
		}
	}

	if ( isset( $wp_madx_cache_query['is_404'] ) && false == apply_filters( 'wpmadxcache_404', false ) ) {
		$new_cache = false;
		if ( isset( $GLOBALS['wp_madx_cache_debug'] ) && $GLOBALS['wp_madx_cache_debug'] ) {
			wp_cache_debug( '404 file not found not cached', 2 );
			wp_cache_add_to_buffer( $buffer, 'Page not cached by WP madx Cache. 404.' );
		}
	}
	...
	if ( ! preg_match( apply_filters( 'wp_cache_eof_tags', '/(<\/html>|<\/rss>|<\/feed>|<\/urlset|<\?xml)/i' ), $buffer ) ) {
		...
		if ( isset( $GLOBALS['wp_madx_cache_debug'] ) && $GLOBALS['wp_madx_cache_debug'] ) {
			...
			wp_cache_add_to_buffer( $buffer, 'Page not cached by WP madx Cache. No closing HTML tag. Check your theme.' );
		}
	}
	...
	if ( ! wp_cache_writers_entry() ) {
		wp_cache_add_to_buffer( $buffer, 'Page not cached by WP madx Cache. Could not get mutex lock.' );
		wp_cache_debug( 'Could not get mutex lock. Not caching.', 1 );
		return wp_cache_maybe_dynamic( $buffer );
	}

	if ( $wp_cache_not_logged_in && isset( $wp_madx_cache_query['is_feed'] ) ) {
	...
	$dir           = get_current_url_madxcache_dir();
	$madxcachedir = $cache_path . 'madxcache/' . preg_replace( '/:.*$/', '', $home_url['host'] );
	if ( wpsc_is_get_query() || isset( $wp_madx_cache_query['is_feed'] ) || ( $madx_cache_enabled == true && is_dir( substr( $madxcachedir, 0, -1 ) . '.disabled' ) ) ) {
		wp_cache_debug( 'madxcache disabled: GET or feed detected or disabled by config.', 2 );
		$madx_cache_enabled = false;
	}

	$tmp_wpcache_filename = $cache_path . uniqid( mt_rand(), true ) . '.tmp';

	if ( defined( 'WPSC_madxCACHE_ONLY' ) ) {
		$madxcacheonly = true;
		wp_cache_debug( 'wp_cache_get_ob: WPSC_madxCACHE_ONLY defined. Only creating madxcache files.' );
	} else {
		$madxcacheonly = false;
	}
	...
	if ( $madx_cache_enabled ) {
		if ( wp_cache_get_cookies_values() == '' && ! wpsc_is_get_query() ) {
			wp_cache_debug( 'Anonymous user detected. Only creating madxcache file.', 3 );
			$madxcacheonly = true;
		}
	}
	...
	if ( ! $cache_enabled ) {
		...
		wp_cache_add_to_buffer( $buffer, "Page not cached by WP madx Cache. Check your settings page. $cache_error" );
		if ( $wp_cache_mfunc_enabled == 1 ) {
			global $wp_madx_cache_late_init;
			if ( false == isset( $wp_madx_cache_late_init ) || ( isset( $wp_madx_cache_late_init ) && $wp_madx_cache_late_init == 0 ) ) {
				wp_cache_add_to_buffer( $buffer, 'madx Cache dynamic page detected but $wp_madx_cache_late_init not set. See the readme.txt for further details.' );
			}
		}
	...
	if ( ! $madxcacheonly ) {
		...
		if ( ! $fr ) {
			wp_cache_debug( 'Error. madxcache could not write to ' . str_replace( ABSPATH, '', $cache_path ) . $cache_filename, 1 );
			wp_cache_add_to_buffer( $buffer, "File not cached! madx Cache Couldn't write to: " . str_replace( ABSPATH, '', $cache_path ) . $cache_filename );
			wp_cache_writers_exit();
			return wp_cache_maybe_dynamic( $buffer );
		}
	} else {
		$user_info = wp_cache_get_cookies_values();
		$do_cache  = apply_filters( 'do_createmadxcache', $user_info );
		if (
			$madx_cache_enabled &&
			(
				$user_info == '' ||
			...
			$cache_fname        = $dir . madxcache_filename();
			$tmp_cache_filename = $dir . uniqid( mt_rand(), true ) . '.tmp';
			$fr2                = @fopen( $tmp_cache_filename, 'w' );
			if ( ! $fr2 ) {
				wp_cache_debug( 'Error. madxcache could not write to ' . str_replace( ABSPATH, '', $tmp_cache_filename ), 1 );
				wp_cache_add_to_buffer( $buffer, "File not cached! madx Cache Couldn't write to: " . str_replace( ABSPATH, '', $tmp_cache_filename ) );
				@fclose( $fr );
				...
				if ( ! $gz ) {
					wp_cache_debug( 'Error. madxcache could not write to ' . str_replace( ABSPATH, '', $tmp_cache_filename ) . '.gz', 1 );
					wp_cache_add_to_buffer( $buffer, "File not cached! madx Cache Couldn't write to: " . str_replace( ABSPATH, '', $tmp_cache_filename ) . '.gz' );
					@fclose( $fr );
					@unlink( $tmp_wpcache_filename );
					@fclose( $fr2 );
	...
	$added_cache = 0;
	$oc_key      = get_oc_key();
	$buffer      = apply_filters( 'wpmadxcache_buffer', $buffer );
	...
	if ( $wp_cache_mfunc_enabled == 1 ) {
		...
		global $wp_madx_cache_late_init;
		if ( false == isset( $wp_madx_cache_late_init ) || ( isset( $wp_madx_cache_late_init ) && $wp_madx_cache_late_init == 0 ) ) {
			wp_cache_add_to_buffer( $buffer, 'madx Cache dynamic page detected but late init not set. See the readme.txt for further details.' );
		}

		if ( $fr ) { // wpcache caching
			wp_cache_debug( 'Writing dynamic buffer to wpcache file.' );
			wp_cache_add_to_buffer( $buffer, 'Dynamic WPCache madx Cache' );
			fputs( $fr, '<?php die(); ?>' . $buffer );
		} elseif ( isset( $fr2 ) ) { // madxcache active
			wp_cache_debug( 'Writing dynamic buffer to madxcache file.' );
			wp_cache_add_to_buffer( $buffer, 'Dynamic madx Cache' );
			fputs( $fr2, $buffer );
		}
		...
		if ( $fr2 ) {
			wp_cache_debug( 'Writing non-gzipped buffer to madxcache file.' );
			wp_cache_add_to_buffer( $buffer, 'madx cache' );
			fputs( $fr2, $buffer );
		}
		if ( isset( $gzdata ) && $gz ) {
			wp_cache_debug( 'Writing gzipped buffer to madxcache file.' );
			fwrite( $gz, $gzdata );
		}
	}
	...
	if ( $fr2 ) {
		fclose( $fr2 );
		if ( $wp_cache_front_page_checks && $cache_fname == $madxcachedir . $home_url['path'] . madxcache_filename() && ! ( $wp_cache_is_home ) ) {
			wp_cache_writers_exit();
			...
			wp_cache_debug( "Renamed temp madxcache file to $cache_fname", 5 );
			$added_cache = 1;
		}
	}
	if ( $gz ) {
		...
		if ( @filesize( $tmp_cache_filename . '.gz' ) == 0 ) {
			...
			wp_cache_debug( "Renamed temp madxcache gz file to {$cache_fname}.gz", 5 );
			$added_cache = 1;
		}
	}

	if ( $added_cache && isset( $wp_madxcache_cache_list ) && $wp_madxcache_cache_list ) {
		update_option( 'wpmadxcache_count', ( get_option( 'wpmadxcache_count' ) + 1 ) );
		$last_urls = (array) get_option( 'madxcache_last_cached' );
		...
		update_option( 'madxcache_last_cached', $last_urls );
	}
	wp_cache_writers_exit();
...
function prune_madx_cache( $directory, $force = false, $rename = false ) {

	// Don't prune a NULL/empty directory.
	if ( null === $directory || '' === $directory ) {
		wp_cache_debug( 'prune_madx_cache: directory is blank' );
		return false;
	}
	...
	$directory = wpsc_get_realpath( $directory );
	if ( $directory == '' ) {
		wp_cache_debug( "prune_madx_cache: exiting as file/directory does not exist : $dir" );
		return false;
	}
	if ( ! wpsc_is_in_cache_directory( $directory ) ) {
		wp_cache_debug( "prune_madx_cache: exiting as directory is not in cache path: *$directory* (was $dir before realpath)" );
		return false;
	}

	if ( false == @file_exists( $directory ) ) {
		wp_cache_debug( "prune_madx_cache: exiting as file/dir does not exist: $directory" );
		return $log;
	}
	...
	if ( is_dir( $directory ) ) {
		if ( $dh = @opendir( $directory ) ) {
			...
			while ( ( $entry = @readdir( $dh ) ) !== false ) {
				...
				prune_madx_cache( $entry, $force, $rename );
		...
		if ( $oktodelete && ! $rename ) {
			wp_cache_debug( "prune_madx_cache: deleted $directory", 5 );
			@unlink( $directory );
			++$log;
		} elseif ( $oktodelete && $rename ) {
			wp_cache_debug( "prune_madx_cache: wp_cache_rebuild_or_delete( $directory )", 5 );
			wp_cache_rebuild_or_delete( $directory );
			++$log;
		} else {
			wp_cache_debug( "prune_madx_cache: did not delete file: $directory" );
		}
	} else {
			wp_cache_debug( "prune_madx_cache: did not delete file as it wasn't a directory or file and not forced to delete new file: $directory" );
	}
	return $log;
}
...
function wp_cache_phase2_clean_expired( $file_prefix, $force = false ) {
	...
	if ( ( $handle = @opendir( $blog_cache_dir ) ) ) {
		...
		if ( false == $wp_cache_preload_on || true == $force ) {
			wp_cache_debug( "Doing GC on madxcache dir: {$cache_path}madxcache", 2 );
			$deleted = prune_madx_cache( $cache_path . 'madxcache', false, false );
		}
	}
...
function wp_cache_shutdown_callback() {
	global $cache_max_time, $meta_file, $new_cache, $wp_cache_meta, $known_headers, $blog_id, $wp_cache_gzip_encoding, $madxcacheonly, $blog_cache_dir;
	global $wp_cache_request_uri, $wp_cache_key, $cache_enabled, $wp_cache_blog_charset, $wp_cache_not_logged_in;
	global $WPSC_HTTP_HOST, $wp_madx_cache_query;
	...
	if ( ! isset( $response['Content-Type'] ) && ! isset( $response['Content-type'] ) ) {
		...
		if ( isset( $wp_madx_cache_query['is_feed'] ) ) {
			if ( isset( $wp_madx_cache_query['is_sitemap'] ) ) {
				$type  = 'sitemap';
				$value = 'text/xml';
			} else {
		...
		} elseif ( isset( $wp_madx_cache_query['is_rest'] ) ) { // json
	...
	if ( $cache_enabled && ! $madxcacheonly && $new_cache ) {
		...
		if ( wp_cache_writers_entry() ) {
			...
			if ( ( isset( $wp_madx_cache_query['is_feed'] ) || $is_feed ) && ! isset( $wp_madx_cache_query['is_single'] ) ) {
...
function wp_cache_get_postid_from_comment( $comment_id, $status = 'NA' ) {
	global $madx_cache_enabled, $wp_cache_request_uri;
	...
	if ( ! preg_match( '/wp-admin\//', $wp_cache_request_uri ) ) {
		if ( $comment['comment_approved'] == 'delete' && ( isset( $comment['old_comment_approved'] ) && $comment['old_comment_approved'] == 0 ) ) { // do nothing if moderated comments are deleted
			...
			if ( $comment['comment_type'] == '' ) {
				wp_cache_debug( "Moderated comment. Don't delete madxcache file until comment approved.", 4 );
				$madx_cache_enabled = 0; // don't remove the madx cache static file until comment is approved
				define( 'DONOTDELETECACHE', 1 );
			} else {
...
function wp_cache_clear_cache( $blog_id = 0 ) {
	...
	if ( $blog_id == 0 ) {
		...
		prune_madx_cache( $cache_path . 'madxcache/', true );
		prune_madx_cache( $cache_path, true );
	} else {
		wp_cache_debug( "Clearing all cached files for blog $blog_id in wp_cache_clear_cache()", 4 );
		prune_madx_cache( get_madxcache_dir( $blog_id ), true );
		prune_madx_cache( $cache_path . 'blogs/', true );
	}
...
function wpsc_delete_cats_tags( $post ) {
	if ( function_exists( '_deprecated_function' ) ) {
		_deprecated_function( __FUNCTION__, 'WP madx Cache 1.6.3', 'wpsc_delete_post_archives' );
	}
	...
	if ( $categories ) {
		...
		foreach ( $categories as $cat ) {
			prune_madx_cache( get_madxcache_dir() . $category_base . $cat->slug . '/', true );
			wp_cache_debug( 'wpsc_post_transition: deleting category: ' . get_madxcache_dir() . $category_base . $cat->slug . '/' );
		}
	}
	...
	if ( $posttags ) {
		...
		foreach ( $posttags as $tag ) {
			prune_madx_cache( get_madxcache_dir() . $tag_base . $tag->slug . '/', true );
			wp_cache_debug( 'wpsc_post_transition: deleting tag: ' . get_madxcache_dir() . $tag_base . $tag->slug . '/' );
		}
	}
}
...
function wp_cache_post_edit( $post_id ) {
	...
	if ( ! apply_filters( 'wp_madx_cache_clear_post_cache', true, $post ) ) {
		return $post_id;
	}
	...
	if ( $wp_cache_clear_on_post_edit ) {
		wp_cache_debug( "wp_cache_post_edit: Clearing cache $blog_cache_dir and {$cache_path}madxcache/ on post edit per config.", 2 );
		prune_madx_cache( $blog_cache_dir, true );
		prune_madx_cache( get_madxcache_dir(), true );
...
function wp_cache_post_id_gc( $post_id, $all = 'all' ) {
	...
	if ( $all ) {
		prune_madx_cache( $dir, true, true );
		do_action( 'gc_cache', 'prune', $permalink );
		@rmdir( $dir );
		$madxcache_home = get_madxcache_dir();
		wp_cache_debug( "wp_cache_post_id_gc clearing cache in {$madxcache_home}page/." );
		prune_madx_cache( $madxcache_home . 'page/', true );
...
function wp_cache_post_change( $post_id ) {
	global $file_prefix, $cache_path, $blog_id, $madx_cache_enabled, $blog_cache_dir, $wp_cache_refresh_single_only;
	...
	// Delete madxcache files whenever a post change event occurs, even if madxcache is currently disabled.
	...
	if ( $all == true ) {
		wp_cache_debug( 'Post change: madxcache enabled: deleting cache files in ' . $dir );
				...
				if ( strpos( $file, '.html' ) ) {
					...
					if ( $post_id > 0 && $meta ) {
						...
						if ( $meta['blog_id'] == $blog_id && ( ( $all == true && ! $meta['post'] ) || $meta['post'] == $post_id ) ) {
							...
							@unlink( $blog_cache_dir . 'meta/' . $file );
							@unlink( $blog_cache_dir . $file );
							if ( false == $madxcache_files_deleted && $madx_cache_enabled == true ) {
								wp_cache_debug( "Post change: deleting madxcache files for {$permalink}" );
								wpsc_rebuild_files( $dir . $permalink );
								$madxcache_files_deleted = true;
						...
						@unlink( $blog_cache_dir . 'meta/' . $file );
						@unlink( $blog_cache_dir . $file );
						if ( $madx_cache_enabled == true ) {
							wp_cache_debug( "Post change: deleting madxcache files for {$meta[ 'uri' ]}" );
...
function wp_cache_gc_cron() {
	...
	update_option( 'wpmadxcache_gc_time', time() );
	...
	if ( false === ( $num = wp_cache_phase2_clean_expired( $file_prefix ) ) ) {
		...
		update_option( 'wpmadxcache_gc_time', time() - ( $cache_time_interval - 10 ) ); // if GC failed then run it again in one minute
		$msg .= __( 'Cache expiry cron job failed. Job will run again in 10 seconds.', 'wp-madx-cache' ) . "\n";
	}
	if ( time() - $start > 30 ) {
		wp_cache_debug( "Cache Expiry cron job took more than 30 seconds to execute.\nYou should reduce the Expiry Time in the WP madx Cache admin page\nas you probably have more cache files than your server can handle efficiently.", 1 );
		$msg .= __( 'Cache expiry cron job took more than 30 seconds. You should probably run the garbage collector more often.', 'wp-madx-cache' ) . "\n";
	}

	if ( $cache_gc_email_me ) {
		if ( $msg != '' ) {
			$msg = "The following warnings were generated by the WP madx Cache Garbage Collector:\n" . $msg;
		}

		$msg = "Hi,\n\nThe WP madx Cache Garbage Collector has now run, deleting " . (int) $num . " files and directories.\nIf you want to switch off these emails please see the WP madx Cache Advanced Settings\npage on your blog.\n\n{$msg}\nRegards,\nThe Garbage Collector.";

		wp_mail( get_option( 'admin_email' ), sprintf( __( '[%1$s] WP madx Cache GC Report', 'wp-madx-cache' ), home_url() ), $msg );
	}
	@unlink( $gc_flag );
...
function schedule_wp_gc( $forced = 0 ) {
	...
	if ( $cache_schedule_type == 'interval' ) {
		...
		$last_gc = get_option( 'wpmadxcache_gc_time' );
