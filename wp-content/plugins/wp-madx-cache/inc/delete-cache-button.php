...
function wpsc_admin_bar_render( $wp_admin_bar ) {
	...
	if ( ( is_singular() || is_archive() || is_front_page() || is_search() ) && current_user_can(  'delete_others_posts' ) ) {
		...
		$wp_admin_bar->add_menu( array(
					'parent' => '',
					'id' => 'delete-cache',
					'title' => __( 'Delete Cache', 'wp-madx-cache' ),
					'meta' => array( 'title' => __( 'Delete cache of the current page', 'wp-madx-cache' ) ),
					'href' => wp_nonce_url( admin_url( 'index.php?action=delcachepage&path=' . rawurlencode( $path ) ), 'delete-cache-' . $path . '_0', 'nonce' )
					) );
	}

	if ( is_admin() && ( wpmadxcache_site_admin() || current_user_can( 'delete_others_posts' ) ) ) {
		$wp_admin_bar->add_menu( array(
					'parent' => '',
					'id' => 'delete-cache',
					'title' => __( 'Delete Cache', 'wp-madx-cache' ),
					'meta' => array( 'title' => __( 'Delete madx Cache cached files', 'wp-madx-cache' ) ),
...
function wpsc_delete_cache_directory() {
	...
	$path = $valid_nonce ? realpath( trailingslashit( get_madxcache_dir() . str_replace( '..', '', preg_replace( '/:.*$/', '', $req_path ) ) ) ) : false;
	...
	if ( $path ) {
		...
		$madxcachepath = realpath( get_madxcache_dir() );

		if ( false === wp_cache_confirm_delete( $path ) || 0 !== strpos( $path, $madxcachepath ) ) {
