<?php
function wp_madxcache_searchengine( $string ) {
...
add_cacheaction( 'wp_cache_get_cookies_values', 'wp_madxcache_searchengine' );

function searchenginemadxcache( $user_info ) {
...
function searchengine_phase2_actions() {
	...
	if( $cache_no_adverts_for_friends == 'yes' ) {
		add_filter( 'do_createmadxcache', 'searchenginemadxcache' );
	}
}
add_cacheaction( 'add_cacheaction', 'searchengine_phase2_actions' );

function wp_madxcache_searchengine_admin() {
	...
	if(isset($_POST['cache_no_adverts_for_friends']) && $valid_nonce) {
		$cache_no_adverts_for_friends = $_POST['cache_no_adverts_for_friends'] == __( 'Disable', 'wp-madx-cache' ) ? 'no' : 'yes';
	...
	?>
		...
		<h4><?php _e( 'No Adverts for Friends', 'wp-madx-cache' ); ?></h4>
		<form name="wp_manager" action="" method="post">
		<label><input type="radio" name="cache_no_adverts_for_friends" value="1" <?php if( $cache_no_adverts_for_friends == 'yes' ) { echo 'checked="checked" '; } ?>/> <?php _e( 'Enabled', 'wp-madx-cache' ); ?></label>
		<label><input type="radio" name="cache_no_adverts_for_friends" value="0" <?php if( $cache_no_adverts_for_friends == 'no' ) { echo 'checked="checked" '; } ?>/> <?php _e( 'Disabled', 'wp-madx-cache' ); ?></label>
		<p><?php _e( '', 'wp-madx-cache' ); ?></p><?php
		echo '<p>' . __( 'Provides support for <a href="http://ocaoimh.ie/no-adverts-for-friends/">No Adverts for Friends</a>.', 'wp-madx-cache' ) . '</p>';
		if ( isset( $changed ) && $changed ) {
			if ( 'yes' == $cache_no_adverts_for_friends )
				$status = __( "enabled" );
			else
				$status = __( "disabled" );
			echo "<p><strong>" . sprintf( __( "No Adverts for Friends support is now %s", 'wp-madx-cache' ), $status ) . "</strong></p>";
		}
...
add_cacheaction( 'cache_admin_page', 'wp_madxcache_searchengine_admin' );
